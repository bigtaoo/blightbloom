...
| File | Role | Tested |
|------|------|--------|
| `scripts/build.mjs` | esbuild-bundles the five entrypoints into `dist/*.mjs`, resolving the `@dd/engine` / `@dd/game/*` / `@dd/net/*` aliases at BUILD time; `ws` and `mongodb` stay external (the driver reaches for built-ins through a dynamic `require` — inlined, the build PASSES and every service dies at boot). `node:sqlite` left the list on 2026-09-15 because no source file imports it, and its absence from every bundle is asserted. Exports `entries`/`external`/`target` so the tests read the real values. | ✅ `test/deploy.bundle.test.ts` |
