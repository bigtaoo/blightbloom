...
| File | Role | Tested |
|------|------|--------|
| `scripts/build.mjs` | esbuild-bundles the five service entrypoints into `dist/*.mjs`, resolving the `@dd/engine` / `@dd/game/*` / `@dd/net/*` aliases at BUILD time; `ws` and `mongodb` stay external (the driver reaches for built-ins through a dynamic `require` — inlined, the build PASSES and every service dies at boot). Each entry carries `service: true|false`: a bundle nothing runs on a schedule (the one-time cutover tool was one, for a day) has no compose `command:` to be cross-checked against, and the flag is what says so instead of an exemption list. `node:sqlite` left the `external` list on 2026-09-15 because no source file imports it, and its absence from every bundle is asserted. Exports `entries`/`external`/`target` so the tests read the real values. | ✅ `test/deploy.bundle.test.ts` |
