...
| `src/mongo.ts` | The cluster connection (design/16-accounts.md). One pooled `MongoClient`, connected at boot by every `main.ts` before it binds a port, over FOUR logical databases — `accounts`/`billing`/`analytics`/`ops`, exactly where four `node:sqlite` files were until 2026-09-15. `BB_MONGO_URI` has no default, deliberately: the failure mode of a wrong one is a service that comes up healthy against an empty database. | env | ✅ `test/mongo.test.ts`, `test/mongo.semantics.test.ts` (four SERVER behaviours the port re-bought) |
| `src/db.ts` / `src/AuthService.ts` | The control plane's collections: `accounts`/`sessions`/`ratings`/`metaState`/`entitlements`/`ratingReports` (design/16-accounts.md). Foreign keys did NOT survive the port and have no equivalent; the CHECKs did, as `$jsonSchema`/`$expr` validators. | `Db` | ✅ `test/db.test.ts`, `test/AuthService.test.ts` |
...
| File | Role | I/O? | Tested |
|------|------|------|--------|
| `src/billingDb.ts` | billsvc's OWN logical database: `orders`/`receipts`/append-only `ledger`/`deliveries`, plus 8.5's `webhookEvents`/`reviewQueue`. Six collections, and a test asserts exactly those and no more. Deliberately a separate DATABASE rather than a collection prefix — money gets its own, and a shared handle is how that gets undone. `orders.platformTxnId`'s unique index is PARTIAL, which is the port's one genuinely dangerous difference. | `Db` | ✅ `test/billingDb.test.ts` |
...
| File | Role | Tested |
|------|------|--------|
| `scripts/build.mjs` | esbuild-bundles the five service entrypoints — plus `migrate.mjs`, the one-time cutover tool, marked `service: false` — into `dist/*.mjs`, resolving the `@dd/engine` / `@dd/game/*` / `@dd/net/*` aliases at BUILD time; `ws` and `mongodb` stay external (the driver reaches for built-ins through a dynamic `require` — inlined, the build PASSES and every service dies at boot). `node:sqlite` left the `external` list on 2026-09-15 because no source file imports it, and which bundles carry it is asserted by name. Exports `entries`/`external`/`target` so the tests read the real values. | ✅ `test/deploy.bundle.test.ts` |
...
| `src/adminsvc/dbs.ts` | The three player-data handles, and the boot-time WRITE PROBE that is now all that stands behind decision B1. `readOnly: true` handles behind `:ro` mounts were two enforcement layers neither of which was our code; an Atlas role is configuration this repo cannot see, so the process attempts a real write to each database and refuses to start unless the server refuses. A check at one instant rather than a capability — weaker, and said so everywhere it appears. | `Db` | ✅ `test/adminsvc.dbs.test.ts` (both probe arms, the refusal, and the loud `BB_ADMIN_ALLOW_WRITABLE` hatch) |
...
| `src/flags/store.ts` | The `ops` store's `flags` collection. A document means "overridden"; ABSENCE means "as shipped", so clearing DELETES rather than writing today's default where it would go stale. C1 is enforced on both sides: `setFlag` will not write an unknown name and `readOverrides` will not return one. `BB_OPS_FLAGS_ENABLED` is the opt-in that a missing file path used to be. | `Db` | ✅ `test/flags.store.test.ts` |
...
| File | Role | I/O? | Tested |
|------|------|------|--------|
| `src/backup/config.ts` | `BACKUP_STORES` (compiled in, `satisfies readonly StoreName[]`, `ops` deliberately excluded) + `BB_MONGO_URI`/`BB_BACKUP_DIR`/`_INTERVAL_HOURS`/`_KEEP`. Mostly refusals: no cluster, or a zero/garbage interval, is a startup error, because a backup worker with nothing to do looks exactly like a working one. An EMPTY var counts as unset. | env | ✅ `test/backup.config.test.ts` |
| `src/backup/snapshot.ts` | One dump per logical database: gzipped NDJSON in Extended JSON (`zcat`- and `mongoimport`-readable, and lossless for the ObjectIds that carry an ordering), read back and line-counted before an atomic rename. The timestamp lives in the FILENAME, since a restore or an `rsync -a` rewrites mtimes — and `SNAPSHOT_RE` deliberately does not match the retired `.db.gz` names. NOT point-in-time consistent across collections; see the header. | `Db` | ✅ `test/backup.snapshot.test.ts` |
...
| `src/backup/runner.ts` | One cycle, sources read SEQUENTIALLY (three concurrent full-collection scans is the load this worker exists not to be). Per-source failures are recorded, never thrown, so one unreachable database cannot cost the others their backup. Publishes `status.json` atomically; the health verdict read back off it is unhealthy when any source failed OR the last cycle is too old. | `Db` | ✅ `test/backup.runner.test.ts` |
| `src/backup/main.ts` | Two modes, one bundle: the loop (one cycle immediately, then every interval) and `--health`, which is the container's healthcheck and returns BEFORE connecting, so a network blip cannot turn the container red over a condition the last cycle already recorded. Refuses to start (exit 2) rather than idle. | env | ✅ `test/backup.main.test.ts`, `test/deploy.bundle.test.ts` |
| `src/migrate/` + `scripts/migrateFromSqlite.ts` | **The one-time migration, and the last `node:sqlite` import in this repository.** Sixteen legacy tables → their collections, deterministic `_id`s so a re-run is an upsert, and two guards against a re-run AFTER the cutover. `--dry-run` runs every mapping without writing. Ships as `dist/migrate.mjs` (`node migrate.mjs --dir=/data`) — it reaches the box in the image or not at all. DELETE BOTH once the data has moved and been verified. | file + `Db` | ✅ `test/migrate.test.ts` (every mapping, both guards, against a real cluster) + `test/deploy.bundle.test.ts` (the shipped BUNDLE, against real `.db` files) |
...
**Billing (ROADMAP 8.3/8.4/8.5, design/19 §4/§5/§7):** billsvc reads its own logical database on
the cluster (`store('billing')`) — a separate DATABASE rather than a collection prefix, because a
shared handle is how a later refactor quietly re-merges two planes this decision separated on
purpose. It used to be `BB_BILLING_DB_PATH`, deliberately a different variable from `BB_DB_PATH`
so that one operator setting one value could not point both planes at one file; the cluster
replaces that with a name neither plane can reach past. `BB_BILLING_DEV_STUB=1` enables the
