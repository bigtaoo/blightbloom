 ...
 * would put a dozen aggregation pipelines — against a database another process is writing —
 * on a path triggered by something outside this project. So the job computes on ITS OWN
 * schedule and `/metrics` reads {@link RollupJob.metrics}, which is a field read.
 *
 * The cost of that trade is now larger than it was and is stated rather than discovered:
 * against `node:sqlite` the first cycle finished before {@link startRollupJob} returned, so
 * the window in which `/metrics` reports no analytics gauges was zero. The cycle is
 * asynchronous now, so that window is real — one round trip to the cluster wide. It is
 * still the correct answer during it (see the absent-versus-zero rule below), and
 * {@link RollupJob.ready} is offered for the one caller that needs to know the first cycle
 * has landed: a test.
 *
 * ## Hourly, not daily
 *
 * The work is idempotent (`persistRollup` upserts by key) and cheap (a handful of indexed
 * counts), so the interval is chosen for a different reason: a DAILY timer in a process
 * that restarts on every deploy is a timer that can miss a day entirely, and the miss is
 * invisible — the document is simply absent, which reads exactly like a quiet day. Hourly
 * means a restart costs at most an hour of staleness and the day boundary is picked up on
 * its own.
 *
...
export interface RollupJob {
  ...
  /** Run a cycle now. Exposed for tests and called once by {@link startRollupJob}. Never
   *  rejects: a failed cycle is logged and the cache is left alone. */
  runOnce: () => Promise<void>;
  /** The cycle {@link startRollupJob} kicked off before returning. Awaiting it is how a test
   *  observes the "serves real gauges at once" property without racing the first round trip;
   *  production ignores it, because "no gauges yet" is already the correct answer. */
  ready: Promise<void>;
  stop: () => void;
}
 ...
 *
 * **Persist BEFORE pruning**, and the honest version of why: the rollup reads documents the
 ...
 */
export async function runRollupCycle(deps: RollupJobDeps): Promise<RollupMetric[]> {
  const now = deps.now ?? Date.now;
  const today = dayKey(now());
  const rows = await persistRollup(deps.db, today, now());
  const pruned = await prune(deps.db, today);
  const metrics = await rollupMetrics(deps.db, today);
...
/**
 * Start the job. Kicks off one cycle before returning, so a freshly started process serves
 * real gauges as soon as the cluster answers rather than after an hour.
 *
 ...
 * healthy while it slowly goes stale. That is also why {@link RollupJob.runOnce} resolves
 * rather than rejecting: the timer below calls it with no `catch`, and a rejecting version
 * would turn one bad cycle into an unhandled rejection.
 */
export function startRollupJob(deps: RollupJobDeps): RollupJob {
  ...
  const runOnce = async (): Promise<void> => {
    try {
      latest = await runRollupCycle(deps);
    } catch (e) {
  ...
  // `runOnce` itself, not a wrapper that discards its promise: it never rejects (see above),
  // so the timer needs no `catch`, and handing the function over intact is what lets a test
  // driving a fake timer AWAIT the cycle it just triggered instead of polling for it.
