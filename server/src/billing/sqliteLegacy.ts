/**
 * Split of `billingDb.ts` (2026-09-15, the MongoDB port): the RETIRED `node:sqlite` file,
 * kept alive only for the consumers that have not been ported yet.
 *
 * THE BILLING PLANE NO LONGER OPENS THIS. `billsvc` reads and writes the six collections in
 * `collections.ts`/`schema.ts`; nothing under `src/billsvc/` imports this module any more.
 * What still does is the ops console (`adminsvc/dbs.ts`, which mounts the three player-data
 * files read-only), the backup runner's source list, and `scripts/seedOpsDemo.ts` which
 * authors a demo file for that console. Those belong to a later stage of the migration, and
 * deleting the opener before they move would break a plane this pass is not allowed to
 * touch — so it stays, marked, and goes when its last caller does.
 *
 * Everything below is unchanged from the pre-port file. The table comments are the record of
 * what each guarantee USED to rest on; `schema.ts` states where each of them lives now, and
 * is the file to read for the shipped rules.
 *
 * Six tables — four for the money, two for the operations around it:
 *
