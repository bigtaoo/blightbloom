...
const base = { DDU_DB_PATH: '/sources/matchsvc/accounts.db' };
...
describe('readBackupConfig — sources', () => {
  it('takes both databases, in declaration order, from the owning services’ own var names', () => {
    const cfg = readBackupConfig({
      DDU_DB_PATH: '/sources/matchsvc/accounts.db',
      DDU_BILLING_DB_PATH: '/sources/billsvc/billing.db',
    });
    ...
    expect(SOURCE_VARS).toEqual(['DDU_DB_PATH', 'DDU_BILLING_DB_PATH']);
  });

  it('accepts either one alone — billing exists before accounts does not', () => {
    expect(readBackupConfig({ DDU_BILLING_DB_PATH: '/b/billing.db' }).sources).toEqual(['/b/billing.db']);
  });
  ...
  it('treats an EMPTY var as absent, and so refuses on two empty ones', () => {
    ...
    // `DDU_DB_PATH:` and no value produces it.
    expect(() => readBackupConfig({ DDU_DB_PATH: '', DDU_BILLING_DB_PATH: '   ' })).toThrow(
      /no databases to back up/,
    );
  });

  it('trims a stray space rather than opening " /data/x.db"', () => {
    expect(readBackupConfig({ DDU_DB_PATH: ' /data/x.db ' }).sources).toEqual(['/data/x.db']);
  });

  it('refuses a source INSIDE the backup directory — a copy of a copy every cycle', () => {
    expect(() => readBackupConfig({ DDU_DB_PATH: '/backups/accounts.db' })).toThrow(/lies inside/);
  });
});
...
describe('readBackupConfig — schedule and retention', () => {
  ...
  it('takes a fractional interval — the knob an operator turns to test a change', () => {
    expect(readBackupConfig({ ...base, DDU_BACKUP_INTERVAL_HOURS: '0.5' }).intervalMs).toBe(1_800_000);
  });
  ...
  it('refuses a zero, negative or unparseable interval instead of clamping it', () => {
    ...
    for (const value of ['0', '-1', 'nightly', 'NaN', '']) {
      const env = { ...base, DDU_BACKUP_INTERVAL_HOURS: value };
      if (value === '') {
  ...
  it('refuses a fractional keep count — "keep 2.5 snapshots" has no meaning', () => {
    expect(() => readBackupConfig({ ...base, DDU_BACKUP_KEEP: '2.5' })).toThrow(/positive integer/);
    expect(readBackupConfig({ ...base, DDU_BACKUP_KEEP: '3' }).keep).toBe(3);
  });

  it('refuses keep=0 — that is "back up and immediately delete"', () => {
    expect(() => readBackupConfig({ ...base, DDU_BACKUP_KEEP: '0' })).toThrow(BackupConfigError);
  });
});
