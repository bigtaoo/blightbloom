 ...
 *
 * There was a sixth bundle for one day, and the reason is worth keeping after it: the
 * cutover runbook told an operator to run the migration's TypeScript source under `tsx`,
 * inside an image built by `COPY dist/*.mjs ./` that has no `scripts/`, no TypeScript and
 * no `tsx`. The migration's LOGIC had tests against a real cluster; the way it reached a
 * production box had none, so the suite was green and the command was unrunnable. It was
 * packaged as `dist/migrate.mjs`, run against the live box on 2026-09-16, and deleted with
 * `src/migrate/` — along with the `describe` here that ran it. What is left of that is the
 * `service` flag on a build entry and the manifest test that reads it, so the next tool
 * bundle lands with somewhere to be checked rather than an exemption.
 *
...
describe('the built bundles are self-contained', () => {
  it('emits exactly one file per entrypoint', () => {
    ...
    // Every bundle here is a process compose runs. A future entry that is a TOOL instead
    // must say so with `service: false` — `deploy.manifests.test.ts` cross-checks these
    // names against compose's `command:` lines and would otherwise read it as a service
    // nobody wired up.
    expect(entries.every((e: { service: boolean }) => e.service)).toBe(true);
  });
  ...
  it('kept exactly the declared externals external', () => {
    ...
    // reaching for one.
    //
    // `node:sqlite` was the third entry until 2026-09-15. Its absence from every bundle is
    // the sharpest evidence available that the port is complete rather than half-done: one
    // surviving `openBillingDb`-shaped import anywhere under `src/` would put the builtin
    // straight back into whichever bundle reached it, and this list is exact. The one-time
    // migration was the single exception, for one day, and left with the data it carried.
