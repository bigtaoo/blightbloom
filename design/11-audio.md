...
- **`audio/audioPipeline.test.ts`** — real `EventReactor` → real `WebAudio` → real mixer/bank/catalogue, with only the `AudioContext` and the byte read faked. It asserts the property no unit owns: a frame of real `GameEvent`s reaches the **shipped samples**, not the fallback (a frame containing every engine cue plays a decoded voice for each and exactly one synthesised — `status.burn`). It is the automated form of the browser measurement in the status block.
...
- **A source-level guard on every entry point** rides along in the pipeline test: that each calls `audio.preload()`, that none AWAITS it, and that the WeChat one does it after the host swap. The enumeration is three long since 2026-09-07 (`main.ts` / `main.wechat.ts` / `main.crazygames.ts`, design/20) and adding a fourth target without adding it there is the one way to slip past this. Crude on purpose — every entry is a top-level `boot()` script with no seam, and every other test builds the backend itself, so deleting that one line would leave the whole suite green and the shipped set silently unloaded.
...
- **Attached at boot, in every entry**, beside `audio.preload()` — the composition root, where the device is created. A source-level test guards that line in `main.ts`/`main.wechat.ts`/`main.crazygames.ts` for the same reason the preload guard exists: nothing else in the suite runs `boot()`, so its absence would be green everywhere and silent in the game. (It also lives there rather than in `Game`'s constructor because `Game.ts` sits at exactly the 500-line convention's limit — no longer *tracked* as an offender since the 2026-09-03 split, but with zero headroom, so the length gate forbids growing it either way — and the placement is better for it, not a concession.)
...
- **`Game.ts` was not touched.** The per-frame call sits in `GameLoop.update`, ahead of the
  ...
  module sink (`setMusicAudio`, attached at boot in every entry) rather than a constructor dep —
  ...
  shape for something attached at boot in every entry.)*
