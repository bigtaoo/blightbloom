...
- **Battle royale: 8-player, squad or solo, elimination + shrinking zone.** Not a symmetric team arena. ✅ **Shipped (`ENGINE_VERSION` 30):** squads are chunks of `SQUAD_SIZE` (4) seats sharing one `teamId`, formed via a pre-formed party invite code (`server/src/PartyService.ts`) or filled with solo queuers/bots when a party is smaller than a squad (the same room code also makes a two-member CO-OP party since 2026-09-26 — `partyCapacity(mode)` in `client/src/game/match/partyShape.ts` — and a party of either mode is only seated once all its members are queued, [volume 96](roadmap/96-2026-09-26-coop-party-matchmaking.md)); **the room code is six decimal digits** since 2026-09-21 (was five characters of a no-0/O/1/I alphabet — digits are dictatable and typable in all eight shipped locales and let a phone show a keypad, at the cost of a 1M rather than 33.5M keyspace; `randomCode` in `server/src/routes/party.ts`, and `PartyService.create` redraws a code already in use up to `CODE_DRAW_ATTEMPTS` times rather than looping forever). **That keyspace is what the two `/party/*` per-IP budgets defend, from opposite sides** (2026-09-22): `JOIN_RATE_LIMIT` bounds GUESSING — 120 in ten minutes, spent before the body is read and charged on a MISS as well as a hit, since a walk over 10^6 is almost entirely misses, which puts one caller's pass over the whole space at roughly two months, slower than the 10-minute TTL of the parties being walked toward — and `CREATE_RATE_LIMIT` bounds MINTING, 60 in ten minutes, because the ratio of 1M codes to a live set is safe only while the live set stays small and the live set is whatever the create route was allowed to mint. Its window is deliberately the party TTL, which makes the budget also the ceiling on how many parties one address can hold at once; any seat count squads don't cleanly divide into (including exactly `SQUAD_SIZE` itself — one squad covering the whole match would leave nobody able to fight) stays the original one-`teamId`-per-seat free-for-all.
...
`buildArenaSpecs(presetId, skinId)` (`09`) scales **both halves of the fight together** — a character's `(maxHp, maxShield)` and the landing-kit/arena-loot weapons' damage/handling — so relative time-to-kill matches PvE's feel at a bigger absolute number range (PvE's 3–10ish HP pool leaves no room for a shrinking-zone DoT curve to matter; see `05`). This is a **second tuning pass over the same `SkinDef`/`WeaponSpec` content** — a second COLUMN on each character (and a scaled copy of each weapon), never a second roster, and never a change to the PvE numbers themselves. ✅ **Shipped and actually wired**, not just built in isolation: `GameState.buildSeat` (`ENGINE_VERSION` 19→20) branches on `config.arena` to call `buildArenaSpecs`, so a real PvP seat's weapons/`maxHp`/`maxShield` come from the scaled kit — `seat.loadout` (persistent PvE gear) is structurally never read for it. **The two halves are scaled differently, and that is the 2026-09-21 correction (`ENGINE_VERSION` 67).** Weapon damage is still `Math.round(damage × PVP_SCALE_FACTOR)`, currently `5`. The POOLS are no longer derived from it at all — each character authors its arena pair directly (`SkinDef.pvp`): vanguard 30/**16**, skirmisher 15/**30**, juggernaut **75**/0 (55 until `ENGINE_VERSION` 77, see the 2026-09-26 pass below). *(The pools were `Math.round(pool × 5)` until then, and that derivation is the whole reason `vanguard.maxShield` was authored as **3.2** — the only fractional design number in the tree, existing solely so `× 5` landed on 16, and printed at the player on the character screen. `07` has the full account, including why the fraction was inert in PvE. This paragraph also read "vanguard 6/4→30/20, skirmisher 3/8→15/40" until 2026-09-03 — the pre-retune inputs, from before the very rebalance the next paragraph describes.)* What the factor buys is unchanged: pools and weapon damage move TOGETHER, which is what preserves relative TTK; two authored integer columns satisfy that exactly as well as one multiplication did, and either scale can now be re-tuned without dragging the other with it. Still first-pass, per real-play tuning below. **`maxEnergy` is the one character stat that deliberately has NO arena number at all** (`ENGINE_VERSION` 60) — an arena seat carries the raw `SkinDef` pool. `(maxHp, maxShield)` get one *because* weapon damage is scaled alongside them, and that pairing is what preserves relative TTK. `energyCost` is not scaled at all, so giving the pool an arena number too would not preserve any ratio: it would hand every arena seat five times the shots at the same price and delete the ammo economy from PvP outright. `GameState.test.ts` asserts the arena pool against the raw `SkinDef` number rather than against "not the arena one", so the claim survives a retune of either scale. **What this is currently worth is unmeasured, and that is worth stating plainly**: across 8 sampled arena matches no seat's bar ever dropped more than one blaster shot below full, because `landing_basic` is sustainable on regen alone and the bot does not swap to looted frames — so the identical win rates either side of the v60 change are an unmeasured result, not a verified one (the same shape as the melee-share gap `ENGINE_VERSION` 59 named). What bounds the risk meanwhile is arithmetic rather than the bot: capacity buys burst and provably not sustain (`03`/`14`).
...
**The 2026-09-26 pass (`ENGINE_VERSION` 77, [volume 99](roadmap/99-2026-09-26-pvp-balance-bot-guns.md)).** Before the numbers, the instrument. `PvpBotController` never looked at the zone, and the zone looked like three-quarters of all damage dealt. Two things were behind that. The bot chased the nearest opponent wherever that led, so it now leaves a closing room first (`controllers/ai/zoneRetreat.ts`: BFS over the door graph to the nearest room that stays safe, a WARN's closing rooms counting as unsafe) and never follows an opponent into the storm. And the zone kept ticking on **downed** bodies for their whole 900-tick bleedout — HP ran to −46 — although "downed = invulnerable" (`07`, 3.2) held for everything else; `EnvironmentSystem` now skips a downed player. It changed no outcome but it owned the damage total. With both fixed the zone deals ~2.5% of all damage. Measured over 540 matches (2–8 seats × 90 seeds): shipped vanguard/skirmisher/juggernaut 36/46/17%; **juggernaut 75/0 gives 35/44/21%**; 85 and 95 give the same 21% plus ties and timeouts, and trimming the skirmisher's shield to 26 moved nothing. So the remaining gap is the no-regen identity in a multi-fight FFA, not the pool size. **`PVP_SCALE_FACTOR` stays 5** (4 and 6 gave identical win rates: outcomes are set by shots-to-kill breakpoints and the zone, not the absolute scale), and **the zone numbers stay** (they no longer decide anything once the bot walks out). **A parried rival player's bullet now keeps half its damage** (`PVP_DEFLECT_DAMAGE_PERMILLE` 500): without it a blade is a free full-damage counter to any gun in a duel. An enemy's bullet is untouched, so PvE is unaffected. The 500 is the owner's starting number and is **not measured**: the bot never swings its blade, and the sim records zero deflects.
...
### Settlement: a per-seat vote, bounds, and a record ✅ shipped 2026-09-26

The end-of-match half of the same idea, decided by the owner on 2026-09-26 (see Open questions) and built the same day. It hardens what the server does with the seats' `result` messages; nothing here replays anything.

- **The seed is a CSPRNG draw.** `matchsvc.ts` draws each room's seed with `crypto.randomInt(SEED_SPACE)` over the 31-bit range tickets and replay files already carry. It was a counter that started at `Date.now()` and stepped by one per room, so a player who saw one seed could predict the next room's, and with it the drops, spawns and zone.
- **Production needs the ticket secret.** `config.ts`'s `ticketSecret()` throws under `NODE_ENV=production` when `BB_TICKET_SECRET` is unset, and both `createGameserver` and `createMatchsvcServer` call it at build time, so the process refuses to start. It used to warn and fall back to the dev secret published in the same file, which also re-opened the raw-param handshake where a client names its own seat and seed.
- **One vote per seat over the whole tuple** (`server/src/settlement.ts`, `voteSettlement`). Each seat's `{stateHash, winner, placements}` is one vote; squads do not pool, and a bot seat votes like any other. Above `CHECKPOINT_QUORUM` seats, a tuple with a strict majority that is also at least the quorum settles the match, and every seat outside it is a dissenter. At or below the quorum, only unanimity settles. This replaced first-reporter-wins: `reportResult` used to check that every hash matched, then copy `winner`/`placements` from whichever seat reported first, so a seat with the right hash and a forged placement array decided the ladder by being quick, and one divergent hash voided the result for all eight.
- **Bounds before rating** (`checkPvpBounds`). The winner must be a real seat and the lowest seat of its squad (the one `WinConditionSystem` names). The placements must be exactly the seats outside the winning squad, each once. The server's broadcast frame at settlement must be at least `MIN_PVP_SETTLE_FRAME` = 450 (15 s): the 2026-09-26 sweep of 180 bot matches over 2–8 seats never ended before frame 954. A failure settles nothing.
- **Checkpoint kicks reach settlement.** `MatchRoom` keeps every seat `reportCheckpoint` ever kicked, including one that reconnected and then voted with the majority, and puts them in the record.
- **A silent seat times out** (added the same day, [volume 94](roadmap/94-2026-09-26-settle-timeout.md)). The first `result` arms `SETTLE_TIMEOUT_MS` = 30 s. When it runs out, the room settles on the reports it has, and every seat still silent is treated as offline: it casts no vote, and the quorum rule applies to the seats that reported, so a lone reporter in a 1v1 settles. The same happens at once if every seat disconnects after at least one report. Before this, a seat that never reported held the room open forever, so a losing player could keep a result off the ladder by closing the tab.
- **The record.** `SettledMatch.integrity` carries the verdict (`clean`, `partial`, `dissent`, `no_consensus`, `bounds`), the dissenters, the kicked seats, the seats that timed out (`absent`), the settle frame, the seed, and — unless the verdict is clean — the input log. `hashOk` now means "a tuple carried the vote and passed bounds". `partial` (every reporter agreed, but a seat timed out) and `dissent` still rate; `no_consensus` and `bounds` rate nothing. An absent seat is never a suspect: a dropped connection is not a cheat. The gameserver POSTs every non-clean PvP match to matchsvc's internal-key-gated `POST /integrity/report` (`integrityReport.ts` builds it, gzipping the log and dropping it past 4 MB). `IntegrityStore` claims the room id and bumps each suspect account's `suspicion` count in one transaction, so a retried report counts once. A guest or bot suspect is recorded and counts against no one. The ops console's Integrity tab (`21` §3.2) shows both lists.
- **Record, never auto-ban.** Nothing reads a suspicion count but a person. The archived logs, seed and `engineVersion` are there for a replay judge that does not exist yet.

**Out of scope, by the owner's call (2026-09-26): a coordinated majority.** In principle a majority that agrees on a forged tuple wins the vote. The owner ruled this out as a practical threat: players cannot organise it, and each client's hash comes from running the deterministic sim, so a forged tuple would need identically modified clients, not just players agreeing on a story. That residual case is what a server replay would catch, and the replay stays deferred (Open questions).
...
- ~~The PvP HP/weapon scale factor's actual value.~~ **First-pass value shipped:** `PVP_SCALE_FACTOR = 5`. The character-side skew it surfaced (skirmisher/juggernaut win rates) was already rebalanced off the `pvpBalanceSim` harness above (commit `61ccfd8`); re-measured 2026-09-26 with a zone-aware bot (volume 99): the factor stays 5 and the zone curve stays, because neither moves the win rates; what is left is real-play tuning.
...
- Server-shadow-sim escalation and server-authoritative state-sync (this doc's anti-cheat and net sections) are both explicitly deferred until population/revenue justify the server cost — tracked here so it isn't a surprise later, not because it's undecided in principle (`06` already named the same escalation for a different reason).
  - **DECIDED 2026-09-26, by the owner: before any replay, harden the seed-and-consensus layer the way the sibling project `funny` verifies a match — and do no server replay yet.** **Shipped the same day**; see "Settlement: a per-seat vote, bounds, and a record" above for what was built. The list below is the decision as recorded, before the build. That rules out both replay forms for now: the shadow-sim above, and `funny`'s peer judge (a third client re-running the recorded frames), which is a replay run elsewhere. What it does mean, against what the code does today:
    - **The seed is unpredictable.** matchsvc's seed is `Date.now() & 0x7fffffff` plus one per match (`server/src/matchsvc.ts`) — signed into the ticket, but guessable. It becomes a `crypto.randomInt` draw.
    - **Only a signed ticket admits a seat in production.** The legacy query-string handshake (`?roomId&owner&seed&count`, taken when `BB_TICKET_SECRET` is unset) stays dev-only, and a production gameserver without the secret refuses to start.
    - **Settlement compares the whole report, per seat.** `MatchRoom.reportResult` today requires every end hash to match (one divergent seat voids the ladder for all eight) and copies `winner`/`placements` from whichever seat reported first. It becomes a vote over the full `{stateHash, winner, placements}` tuple, **one vote per seat, squads included**: above `CHECKPOINT_QUORUM` seats the majority settles provided it reaches the quorum, and the dissenters are flagged; at or below it, unanimity as now.
    - **Bounds checks before rating.** `placements` is a permutation of every seat but the winner, the winner is in range, and match length is plausible; a failure settles nothing and is recorded.
    - **Checkpoint kicks reach settlement.** A seat `reportCheckpoint` kicked for divergence counts as suspect in the settled record, where today it only reconnects.
    - **Record, never auto-ban.** A divergent match is stored with a per-account suspicion count and surfaced in the ops console; the input log is archived so a later replay has something to judge, but nothing judges it yet.
