...
- ~~**A second NPC: the shop's shopkeeper**~~ — **shipped 2026-09-14**, and the first NPC in the game that stands IN a room rather than on a menu screen. Same body plan and the same universal-mount lore as the Forger above, retuned from smith to trader: a rounder, top-heavy floating core in a worn travelling-cloth mantle, carrying a brass hanging scale and a lantern on the hero's own weapon tethers instead of tools or weapons (`art/npc/prompts.md`). Where the Forger is a corner-anchored sprite on the hub, this one is Y-sorted against the actors in `scene/ShopLayer.ts` and ships to `client/public/environment/npc_shopkeeper.png` with the room fixtures it shares a loader and a preload phase with, not to `ui/`. **It is also the first art asset this doc's "environment desaturated, hazards saturated" rule had to be read in the other direction for.** Every prop assertion in `scene/propArt.test.ts` pushes dressing DOWN into the floor's own 39-49 value band so it never reads as lootable; a character down there is a smudge behind a counter. So the keeper is bounded on BOTH sides instead — `p50` 103, above the stone it stands on and 64 below the drop it must never be mistaken for — with saturation (chroma 49.8, against an elemental body's 151.5) carrying the "not a hazard" half on its own. `scene/npcArt.test.ts` runs both bands over shipped files that must FAIL them, since this generation was accepted first time and left no `_alt` reject to measure.
