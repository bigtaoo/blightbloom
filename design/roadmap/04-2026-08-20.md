...
A screenshot with the block circled — *"the character runs underneath the wall"* — the character standing on the north
...
**Then "add-tests", and the answer was a fourth test file rather than more cases in the three that
...
Live report, screenshot attached, a run circled: *"a door must not be hidden by a tall wall. A door should be clearly visible at all times"* — a
...
Live report, screenshot circling a monster gone behind an interior wall block: *"if only a monster is under the wall
then you cannot see the monster at all"* — if only a monster is under the wall, you can't see the monster at all. The
...
*"on polishing the game scene: so far the focus has been the character and the walls — start on everything else now"* — the scene passes so far had
...
### Tests, and the coverage audit that followed (*"are there tests worth adding"*)
...
This is that pass, prompted by *"the scene and character polish we discussed, especially the 3D-look items — is all of it done?"* and
then *"give me the image prompts"* → *"see whether the images are usable"*.
...
### A second battery, asked for afterwards (*"are there tests worth adding"*) — 18 mutants, 11 survivors
...
Prompted by *"is there still room to improve the game's art?"* → *"in order, please — start by giving me the prompts"*. The audit that question
