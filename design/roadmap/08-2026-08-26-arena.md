...
**A coverage audit over the whole pass (*"are there tests worth adding"*) then found three things the tests written
...
is the exact live complaint the clip was built for in the first place (*"a door must not be hidden by a tall wall. A door should be
clearly visible at all times"*), in a mode that never fed it its doors.
...
- **Still nothing measured on a phone**, and design/04's item 2 is now narrowed rather than closed:
  `LOW` attacks the ~20% of the arena frame that is fill, so expect it to help *less* there than in a
