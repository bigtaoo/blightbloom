...
One user report, with a screenshot of the hero circled in red: *"the shield today is a hoop around the character,
what I want is something like a transparent eggshell wrapping the character completely, not a ring"*.

### 1. The look

The 2026-08-24 pass above un-squashed this shield back into a true circle. That fixed the ellipse and
left the other half of hoop standing: the shader still drew `smoothstep(a, b, dist) * (1.0 -
...
- **1.55 → 1.87 body radii.** "wrapping it completely" includes the mounted weapon: at the smaller radius the gun
...
set the order: *"each one is a locked design rule that shipped half-built, not an optional extra"* — each is a locked
...
behind a wall the occlusion x-ray only partially fades (the 2026-08-21 *"the health bar is blocked by the wall"* fix), and
