...
new file this pass didn't already cover on the first pass ("add tests everywhere" follow-up). 812
...
**"add tests everywhere" follow-up, same day:** closed the coverage gaps a first read-through left —
...
a real door connect, the save-validation gate blocking a bad capstone). **"add tests
everywhere" follow-up, same day:** `DungeonFloorCanvas` (the tool's most complex new
...
**"add-tests" follow-up, same day:** `entranceFromDoor`'s reuse inside
...
## Platform-layer test coverage pass ✅ (2026-08-05, "add tests everywhere")
...
Prompted by "fully audit the project's tests — add what needs adding, delete what needs deleting" (fully audit the project's tests — add what
...
**the deletions (deletions): nothing found.** Every workspace came back clean — no orphaned tests
...
**the additions (additions): ~50 files closed, 891 new tests** (1736 → 2627): `engine/systems/
