...
**WeChat's limits are wider than the note said.** Official subpackage docs: main ≤ 4 MB, a standard
...
found and fixed.** A player report ("this start-game button cannot be clicked" — the start-game button
...
Forge — and got stuck there.** Live report from the simulator: *"after clicking start, the game sticks on the weapon-select
screen, because the button that enters the map is not visible"*. Not an input bug this time, and not WeChat-specific in its
...
`localhost` — the port binds IPv6-only), but every method it exposes is mini-program-shaped and both
...
`WeChat DevTools.exe` is only a ~14 MB launcher stub — matching on the latter reports "the IDE
...
so WeChat reports login succeeded on the phone while the tool stays logged out (only guest mode could
log in). `--version 2.01.2510280 --force` fixes it. The CLI additionally needs
**Settings → Security → Service Port** switched on once by hand, and a mini-GAME project needs a real
registered appid — `touristappid` and an empty appid are both rejected with
`no such AppID (code 10)` even from a logged-in session.
...
reads `AUTO (LOW)` / `AUTO (LOW)`, which is a low-end device reporting itself, model and all. And
