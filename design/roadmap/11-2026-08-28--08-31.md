...
One user report, three symptoms: *"when the character shoots there is no firing animation and no muzzle effect; it looks very stiff,
and the bullet appears very abruptly."* All three were real, and none of them was a missing feature — every
...
Two screenshots and one sentence: *"for the wall in the middle of a room, can the character go only half-way into it?"*, then —
after a first answer that reached for the wrong knob — *"look at the pillars in the room — about half the character's body
is covered by the pillar. Can the walls do the same thing?…in this screenshot the character has run entirely inside the wall."*
...
- a room's south boundary is drawn as a 22 px kerb (`WALL_H_KERB`) whose entire purpose is that a
  character CAN stand tangent to it. Brimming that re-opens v43's *"too close to the wall, it feels sunk in"* from
...
User report: *"the bullet's path now curves from the muzzle round to the front of the character before flying on to the target"*. The mechanism this landed on
...
Third round of the same report — *"there are still drops that cannot be picked up"*, with a circled screenshot of loot
visible through a wall's x-ray fade — and this time the reporter supplied the diagnosis:
*"you need to make sure monsters cannot run into blocked areas, and that drops cannot land in them either"*.
...
*"the starting seed alone is not enough, because where a monster dies depends on where the player is"* — a monster dies where the player
...
*"put F9's function on a button in the UI. Then see whether there are tests worth adding"* — and the first half is the same
...
### "are there tests worth adding" — 21 new tests, and what they were actually for
...
Asked to look at a requirement — *"art needed: 74 pieces of floor-wear decoration at the doorways"* — and the requirement did not
...
*"roadmapthe file is much too long as well, which makes it easy to record things wrongly."*
