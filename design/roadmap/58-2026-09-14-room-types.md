...
> put the shop on floor 4 and the chest several players open together on floor 3. One small chest on every other floor. They do not have to sit on the critical path.
> I will fine-tune the actual room layouts later when I tune the gameplay; just give rooms a few types.
...
**dead end** — *"they do not have to sit on the critical path"*. `alcove` / `court` / `gallery` / `rampart` /
