...
A play report with a red circle drawn round an empty patch of HUD: *"the character can carry a melee
weapon and a gun at the same time, and swap between them where I marked it in the UI. I remember this was implemented once before — take a look"*.
...
Started as a question, not a bug report: *"do you know the mage's magic shield in Legend of Mir?…including the way, when it is hit,
the magic shield deforms — and that was a game 20 years ago"* — and then, once the answer had been talked through:
*"our shield today looks worse than that magic shield does"*, followed by the two sentences that named the whole
job: *"it does not feel wrapped in an eggshell … the ring at the edge is far too solid a line"*.
...
half-peak width 0.10 of the radius.** That number is what *"too solid a line"* is, quantitatively.
...
### 3. "wrapping" is a compositing order, not a shape
...
- **A generated membrane.** The user asked whether the scale texture had to be authored after all
  (*"maybe that scale-detail image has to be generated after all?"*). Answer: the tile is needed, the image model is not. Its two
...
**Second pass, after the user asked *"are there tests worth adding"*.** The honest answer was that the first
...
### 7. *"are there tests worth adding"* — and the answer was seven survivors OUTSIDE the five files
