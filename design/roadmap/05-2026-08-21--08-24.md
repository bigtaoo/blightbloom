...
The user's framing set the priority: *"mainly I do not want to pile up features and then find the art does not hold up, or that many features cannot be done in art at all, or that the client's frame rate is terrible. So I want to validate a level's full art and the client's performance first"* — validate level 1's art and the client's performance BEFORE more feature work. Against that, the parked render-perf list (door recess strokes, pillar base AO, the tether Graphics) is neither art nor features and costs nothing today (render p50 2.1 ms of 16.7), so all of it stays parked; what the level was actually missing was **props**, the last Graphics placeholder in a room.
...
### The add-tests follow-up, and the nine real gaps it found
...
Two user reports in one pass: *"the render frame rate is very low right now. Could you bring over the client profiling system from the funny project??"*
and *"the shield is drawn wrong now — it has become a hoop. I want a spherical shield; the original effect was the right one"*.
...
through the character at gun height — the "hoop" of the report — rather than a bubble enclosing it.
...
Follow-up to the profiler section above, on the user's instruction *"then change the per-actor filter into
one lighting pass over the whole layer"*. The profiler had just measured the per-actor `NormalLitFilter` as the dominant
...
### The add-tests follow-up pass, and the three real gaps it found
...
### The add-tests follow-up, and the one gate that a budget could not hold
