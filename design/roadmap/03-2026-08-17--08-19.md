...
**Report.** "the character does not actually look like the original design. The original design is a round robot with two magnetic arms"
...
was exactly that ("I think I reported this before and it never got fixed. Can you add tests to keep it right?"), and it was fair:
...
**Report, for the third time.** *"I still get focus-fired and killed the moment I enter. You could try writing a simulator. And we
need a level simulator of our own anyway, to balance the difficulty."* — the same focus-fire death `ENGINE_VERSION` 37 (mobs
...
a room hits, how it *reads*. *"1, move the camera down a bit so the viewport holds only the current room, or rather gives the character the best
presentation. 2, bullets must leave the muzzle. 3, monsters must collide with each other. 4, shrink the monsters' sense range and lower their move speed.
5, lower the shield's flicker rate."* Five items, three engine and two render.
...
still unresolved from the original 2D-vs-3Dselection discussion. The audit's answer was that the
...
- ~~**`13`'s rarity-overlay spec**~~ (white→blue→purple→orange→gold without colliding with the five reserved
...
*"the eyes and hands change with the facing now"* — but not form (*"I would like the sense of depth strengthened a bit more"*), and the walls were
still *"there is no sense of height — it is like a picture pasted on the ground"*. Both were fair, and both had the same shape of cause:
...
the passes above had touched — *"the character and the walls read as solid now; some fine polish later should about do it. Have a look at
what room is left to improve in those two areas"* — then, on the answer, *"change them all"*. **Render-only, no `ENGINE_VERSION`
impact.**
...
a second frame of how it should look: *"when the character walks into a wall corner it is too close to the wall and feels sunk into it. Get close to the wall,
...
A screenshot with a red circle round `ember_l1_cell`'s west perimeter run — *"the stretch of wall I circled looks
really odd"* — and then three more reports on the same wall, each rejecting the previous fix. No engine
...
- **Round 2 — *"the vertical wall is drawn straight over the horizontal one"*.** `mergeWallRuns` merges only pairs whose union is a
...
- **Round 3 — *"the middle wall should look like it reaches the bottom of the horizontal wall"*.** Seamless was never the ask. A block's art
...
- **Round 4 — a rectangle drawn on the brick, *"it should cover the area I marked"*.** Measuring the annotation
...
**Then "can you add tests", and the answer was a third test file rather than more cases in the two that
