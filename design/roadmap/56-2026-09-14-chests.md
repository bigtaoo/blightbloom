...
> *"not every room has monsters. Some rooms have a small chest one player can open alone. Some have a big chest with mechanisms around it, one per player who entered the map, and it takes a player standing on each mechanism to open it"* — and, separately, *"blueprints have a chance to drop once the boss is dead. Set the chance for now at5%"*.
...
An audit of the week's work, asked for directly (*"see whether the work from last week has tests worth adding"*). Everything
