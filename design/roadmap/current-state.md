...
>
> **It is drifting right now: the prose below stops at 51, and the shipped version is 78.**
...
> held INTERACT, which also deleted the chest/revive button arbitration v63 had added; 67 the vanguard's shield becoming an integer, with PvP's own authored pools; and the 2026-09-23 content expansion, 68–75 — per-seat schematic and material pickups (68), a halved enemy-HP depth curve (69), two new bosses and the `boss_random` pool (70), the shield-battery and EMP items (71), three independent shop slots and a second counter (72), a skippable floor-layout variant (73), weapon rarity by depth (74), and eleven floor cards (75) — which wrote no work-log volume, see [volume 92](92-2026-09-26-backlog-resync.md); then 76, the shop's buff pick-one-of-three, two more skippable floor variants, the depth curves as `DungeonConfig` fields and juggernaut's 1% boss drop, [volume 97](97-2026-09-26-backlog-close-juggernaut.md); then 77, the PvP pass — no zone damage on a downed body, a parried rival player's bullet at half damage, juggernaut's arena pool 55/0 → 75/0, [volume 99](99-2026-09-26-pvp-balance-bot-guns.md); then 78, the PRNG hashing its seed and its output, [volume 100](100-2026-09-26-prng-mixing.md)), and rewriting another pass's paragraph from outside it is how this file got
...
report ("does the monsters' AI not move? Right now the monsters and the boss only shoot from where they stand") found that `AIDecideSystem` had
...
fire once actually within their own `engageRangeFp`, fixing a live player report ("now that the
monsters are in, the instant I enter the map dozens of them focus-fire me and I die on the spot" — the instant I enter the map, dozens
...
out of the way, from a report that the character *"runs underneath the wall"*; the coverage sweep it produced
...
wall stone. Second, after that had shipped: *"still not right ... the flames when blocked are obvious, but the passable effect is far too weak
"* — the locked leaf is a whole illustrated hazard panel, and no amount of gradient tuning was ever
...
remaining callers). Followed same-day by a "add-tests" pass closing real gaps a first
...
English-canonical i18n system with a Chinese translation (7). **Phase 5 (presentation) is the only
