...
"end-on" the finding named. A boolean answer would have painted a return over the neighbour covering
...
**1. *"the hexagons in the middle of the shield are unreadable; it still looks like a ring"* — and the tile was never hexagons.**
...
**3. *"it looked fine while tuning the effect, but it is quite different in the game itself"*, and this is the general lesson.**
...
**4. *"the shield is a true circle today; an ellipse, a little taller, would probably look more three-dimensional"* — and the comment
...
**5. *"shrink it overall so it sits close to the character, leaving just a small gap — about the diameter of the gun in the picture"* —
...
- The clearance is uniform on ONE axis only and no constant fixes that: the body is round (32 x 32)
  ...
  It is set on the SIDES — the tight axis, where "leave just a small gap" is a clearance that must not close
...
**6. *"when you pick a weapon up, only the matching weapon on the character is replaced. Picking up a blade must not swap out the gun and leave the player holding two blades"*
