...
- **You need not clear a floor** — *design intent, partially shipped (`ROADMAP` B3, filed 2026-09-03; Task 6 "room-layout randomization", `ENGINE_VERSION` 73, 2026-09-23).* How many rooms you can skip is meant to depend on where that floor's extraction room sits: an extraction room mid-floor lets you leave one or two rooms unfought, a natural "greed for the last chest vs. leave safe" micro-decision. *(This micro-decision survived the 2026-09-10 change above: it is about how much of a floor you fight through before descending, not about whether you may leave the RUN there.)* This paragraph used to say fixing it needed BOTH a capstone that may sit at an interior index AND a floor with a genuine bypass — Task 6 ships the second half without the first, on purpose: the capstone still always resolves last (`ExtractionSystem`/`placeAuthoredFloor`/the editor's save gate all assume `rooms[last]`, an invariant this pass left alone rather than relax), but a floor can now fork around an INTERIOR fight room instead. Floor index 1 authors two interchangeable door graphs over the same 7-room roster (`world/rooms/ember.ts EMBER_DUNGEON.floorLayoutVariants`, `DungeonConfig`'s new field): the plain chain (still zero skippable rooms, matching the paragraph below) and a branching layout where `r2_kiln` gets a direct door to `r4_forge` and `r3_span` becomes a dead-end spur off `kiln` instead of sitting on the only path — a run can fight through span for its loot or bypass it entirely. `SpawnSystem` draws one `roomgenPrng` pick between the two per run (the level's first real use of that stream), mirroring the `BOSS_POOL`/`'boss_random'` sentinel's "one well-scoped PRNG choice over otherwise-fixed hand content" shape. No time-pressure mechanic was added anywhere in this — clearing span, like clearing anything else, stays the player's own call, never a clock. Every OTHER floor index still has zero skippable rooms, for the same two reasons that always applied and still apply everywhere the branch above doesn't: the capstone is always the LAST room of a floor, and a room's doors lock as a unit while it holds a live enemy — so on a chain floor every room between spawn and capstone has to be cleared to walk through it. Extending floor index 1's treatment to other floors (and moving the capstone itself to an interior index) is future work, not done by this pass.
...
### Room-layout randomization ✅ (Task 6, `ENGINE_VERSION` 73, 2026-09-23)

A `DungeonConfig` can now offer a POOL of interchangeable `DungeonFloorMap`s for one
floor index instead of a single fixed one — `floorLayoutVariants: Partial<Record<number,
readonly DungeonFloorMap[]>>`. When a floor index has an entry, `SpawnSystem` draws one
`roomgenPrng.nextInt(variants.length)` pick among the pool at floor-generation time
instead of reading `floorMaps[floorIndex]` directly; an index with no entry is unchanged
from before this field existed (reads `floorMaps` directly, draws nothing extra). This
is the same shape `content/enemies.ts`'s `BOSS_POOL`/`'boss_random'` sentinel already
established for boss rooms (Task 2) — one well-scoped PRNG choice layered onto otherwise
fully hand-authored content — applied here to floor TOPOLOGY instead of a single spawn
point, and deliberately not a switch to procedural generation: every variant in a pool is
still hand-placed JSON, `placeAuthoredFloor` still draws nothing, and every variant for a
given index is required to share the exact same room roster in the exact same array
order, so which one a run draws never changes enemy-id allocation (`state.nextId()`,
spawn-order-driven) or the notice-delay tuning derived from it — only door connectivity
differs between variants.

`EMBER_DUNGEON` gives floor index 1 two layouts over its existing 7-room roster: the
plain chain (unchanged, nothing skippable) and `EMBER_L1_FLOOR_2_BRANCH`, where `r2_kiln`
gains a direct door straight to `r4_forge` and `r3_span` moves to a dead-end spur off
`kiln` instead of sitting on the only path through — a run can fight into span for its
loot, or bypass it, entirely the player's call. No time-pressure mechanic accompanies
this or was ever considered for it: clearing every room, including the newly-optional
span, stays exactly as free a choice as clearing a chest/shop side room already was — see
"You need not clear a floor" above for the fuller design-intent account and why this
ships the "genuine bypass" half of that paragraph's two-part fix without the "capstone at
an interior index" half. Every other floor index keeps its single fixed layout; extending
the same treatment further is future work.

`emberLevel1.test.ts` gained a dedicated suite proving the skip is real (BFS over the
door graph with `r3_span` and its doors removed still reaches the capstone) and that the
plain layout has no such skip on any of its other mandatory chain rooms (removing any one
of them DOES disconnect the capstone); `dungeonrun.test.ts` gained a
`floorLayoutVariants` draw-semantics suite mirroring the existing `boss_random` one.
Golden fixture regenerated; any replay reaching floor index 1 of an Ember run diverges
from `ENGINE_VERSION` 73 on.
