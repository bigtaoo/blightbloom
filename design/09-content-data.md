...
- **Deeper floors roll better materials, and (Task 7, `ENGINE_VERSION` 74, 2026-09-23) better weapons too** — `dropTableByDepth` / `materialTierByDepth` (below) shift the material pools by floor. ✅ First-pass shipped: `rollDrop(prng, tier)` tags a material drop with a depth signal (`DeathDropsSystem` passes `state.floorIndex` — a straight identity curve). The configurable `materialTierByDepth` array, and `dropTableByDepth` entirely, are **`ROADMAP` B4**: neither is an unwired field, neither was ever added to the real `DungeonConfig`. So depth buys material TIER and nothing else via THAT mechanism — a deeper floor still rolls materials from the same drop pool as floor 1.
