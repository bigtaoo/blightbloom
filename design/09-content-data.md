...
@dd/engine/
  ...
  content/
    ...
    drops.ts           // DROP_TABLES (no `weapon` entry since v64)      (05)
    shops.ts           // shop stock composition + prices                (05)
...
> **Two more bosses + a third boss trait shipped (Task 3, `ENGINE_VERSION` 70) — a real schema change, not another forward-compatible add.** `PYREFANG` (fire, an omnidirectional ring via the new mob-only `enemynova` weapon spec) and `IRONWARDEN` (physical/armoured) join `BLIGHTLORD` as the roster's bosses, each testing a different axis from the other two: Blightlord races a DoT against a durable HP pool, Pyrefang punishes standing still under a periodic AoE ring (`enrage` alone, faster/wider-perceiving than the roster default so it can reposition), Ironwarden rewards bursting it to a break-point (its own trait, below, alone — no `enrage`, no `onDeathSpawn`). Neither carries `element` — like `brute`/`floater`, a boss/body-form variant is deliberately not one of the four locked variants even though its resist profile echoes one. The floor 5 boss room now spawns a `'boss_random'` sentinel instead of a fixed `blightlord`; `SpawnSystem` resolves it to one `aiPrng` draw off `BOSS_POOL` the first tick that room activates — every other spawn type is unaffected and costs no draw, but this IS the schema/behavior change that bumps the version (a new PRNG draw site on an existing stream, exactly the shape `13`/`27` already established as bump-worthy).
>
> **`armorBreak` — a third boss AI trait, the DEFENSIVE mirror of `enrage`.** `enrage`/`onDeathSpawn` are both one-way latches that make a boss OFFENSIVELY worse to fight past a threshold; `armorBreak: { hpThresholdPermille, resist }` is the opposite direction: the instant hp first crosses it, `resist` is REPLACED by the weaker post-break map, permanently (`WeaponFireSystem.latchArmorBreak`, run right before `latchEnrage` each tick so a boss with both traits — none shipped does — would see its post-break resist already active for that same tick's enrage check). `EnemyActor` gains the matching `armorBreak?`/`armorBroken: boolean` pair, the latter required with the same "stable false default" convention `enraged` follows. A player's read is "the same enemy got easier," never harder — the fx (`armor_break` event, a steel-grey pulse) is deliberately NOT the enrage flash's red, for exactly that reason.

`PLAYER_BASE` holds the stats **shared by all characters** (collision radius, move speed, `WEAPON_SLOTS = 2`, `startWeapons` — the auto-granted starter PAIR, one gun + one melee weapon, which is also the per-kind default table `resolveLoadout` fills a crafted loadout's free slots from, so a run always spawns able to swap (`ENGINE_VERSION` 45; it used to be a single pistol and every ordinary run got only that), `SHIELD_REGEN_DELAY`/`SHIELD_REGEN_INTERVAL`, revive channel length & restored HP); the **chosen `SkinDef`** supplies the per-character `(maxHp, maxShield, maxEnergy)` + `shieldBreak`. At match start the two are merged into the `PlayerActor`; in-run buffs then modify a *copy* via the build layer below — never the shared constant.
...
SkinDef = {
  ...
  maxShield               // soft buffer, absorbed before HP; auto-regens (05/07). An INTEGER,
                          //   like every other authored stat here (v67 — the vanguard shipped a
                          //   3.2 for two months as a rounding artefact of the PvP derivation; 07)
                          //   NOT balanced to equal EHP: 11/0 juggernaut vs 3/6 skirmisher (05)
  pvp: {maxHp,maxShield}  // the PvP-scale pair (15), AUTHORED per character rather than derived
                          //   as round(pool x PVP_SCALE_FACTOR) — that derivation is what forced
                          //   the fraction above. The factor still scales weapon damage (v67)
...
- Buffs are **found in-run** (chests / rooms / shop is the design; the shipped reality is a 6/84 weight on the flat kill `DROP_TABLE`, **the floor-card offer at every checkpoint**, and — since 2026-09-14, `ENGINE_VERSION` 64 — **one line on every shop counter** — `05`'s loot-economy section, `ENGINE_VERSION` 58; five of the seven cards in `balance/floorCards.ts` grant a `RUN_BUFFS` id deliberately rather than defining their own magnitudes, so a card and a floor drop of the same buff are the same strength and `BUFF_CAPS` bounds them together — `ROADMAP` B1/B2) and apply to the player / all held weapons, summed-then-clamped in a fixed order so it stays deterministic (`06`). They are **not** attached to a weapon and never carry out.
...
DungeonConfig = {
  ...
  weaponRarityByDepth?: RarityWeightRow[] // dropTableByDepth's weapon half — SHIPPED 2026-09-26 (v76)
  materialTierByDepth?: number[]       // material quality shift per floor (05) — SHIPPED 2026-09-26 (v76)
}
...
- **Materials are the run's only carry-out** and the meta-forge input. A floor's buffer is **folded into the carry-out bag at each capstone** on descend — which is bookkeeping, not safety: since 2026-09-14 nothing takes that bag off the table before the boss dies (`05`'s "the boss is the only exit"; this line said "banked at extraction rooms (reaching one = a checkpoint)" until then, which read as insurance the checkpoint no longer sells); a run-ending death forfeits the **entire un-extracted carry-out** — this floor's buffer *and* everything banked-but-not-extracted earlier in the run (`05`'s locked wipe rule; this line claimed "only the current floor's" until 2026-09-03). ✅ **Shipped 2026-07-24 (ROADMAP 1.4/1.5, `ENGINE_VERSION` 15, additive):** `state.floorMaterials` (buffer) → `state.bankedMaterials` (carry-out), merged by the new `ExtractionSystem` on EXTRACT/DESCEND; forfeit-on-death is free at both tiers (the buffer is never merged, and the merged bag is never handed to the meta layer — `RunOutcome.lose()` skips `bankRunMaterials`). Gated entirely behind `EngineConfig.floors?` — every config that omits it is untouched. **Per-seat since `ENGINE_VERSION` 68 (blueprint two-tier pass):** both pools moved off `GameState` onto `PlayerActor` — `p.floorMaterials` → `p.bankedMaterials`, one buffer/bank pair per seat, so "whoever picks it up owns it" instead of a squad-shared pile; `RunOutcomeHost.bankRunCarryOut()` (renamed from `bankRunMaterials`) now banks only the local seat's own bags (`14`).
- **Deeper floors roll better materials, and (Task 7, `ENGINE_VERSION` 74, 2026-09-23) better weapons too** — `dropTableByDepth` / `materialTierByDepth` (below) shift the material pools by floor. ✅ First-pass shipped: `rollDrop(prng, tier)` tags a material drop with a depth signal (`DeathDropsSystem` passes `state.floorIndex` — a straight identity curve). The configurable `materialTierByDepth` array, and `dropTableByDepth` entirely, are **`ROADMAP` B4**: neither is an unwired field, neither was ever added to the real `DungeonConfig`. So depth buys material TIER and nothing else via THAT mechanism — a deeper floor still rolls materials from the same drop pool as floor 1. **Closed 2026-09-26 (`ENGINE_VERSION` 76, [volume 97](roadmap/97-2026-09-26-backlog-close-juggernaut.md)):** both curves are real optional `DungeonConfig` fields — `weaponRarityByDepth` (one `RarityWeightRow` of integer percents per floor, read by the chest, boss and shop weapon rolls; validated by `rarityTableProblems`) and `materialTierByDepth` (read through `materialTierForFloor`, plateauing past its end). Absent, each reproduces the shipped behaviour exactly; `EMBER_DUNGEON` sets neither. The buff/heal/coin pool stays depth-blind by decision.
  This line used to add "weapon *finds* stay random at every depth" — no longer true. `content/weaponRarityByDepth.ts rollWeaponId(prng, floorIndex)` now replaces the flat, depth-blind pick at all three places a weapon is found (a chest, `DeathDropsSystem`'s boss-kill drop, a shop's weapon slot): one `weightedIndex` draw over a hand-authored per-floor-index rarity-tier weight table (`common`/`fine` falling and `legend`/`legendary` climbing floor 0→4), built on top of the SAME intrinsic-rarity model this doc's own weapon section describes — no new roll layer, no affix, just which existing weapon a find happens to be shaped by how deep the floor is. Still genuinely random within a tier, and every tier stays reachable at every depth (a floor-0 chest can still find a legendary, just less often than a floor-4 one).
...
- **Character roster** (`02`/`05`): ✅ done (ROADMAP 2.3). `PLAYER_BASE` + two side-grade `SkinDef`s (vanguard, skirmisher) with `(maxHp, maxShield)` + `shieldBreak` shipped first (ROADMAP 0.5); the full 3-character launch roster — vanguard (6/4) / skirmisher (3/6) / **juggernaut** (11HP/0shield, the flat-HP tank) — shipped in Phase 2.3. *(Those are the shipped numbers, as retuned 2026-07-28 against `pvpBalanceSim`; this line said "9HP/0shield" until 2026-09-03, and `14`/`05`/`ROADMAP` 2.3 each carried a different stale triple. Vanguard's shield then read `3.2` until `ENGINE_VERSION` 67 on 2026-09-21, when the PvP pair became authored content and the fraction it had been back-solved from stopped existing — the same pool either way, see `07`.)* *Remaining:* free-vs-paid split, revive timings. (Regen timings currently live in `config.ts` `SHIELD_REGEN_*`, not `PLAYER_BASE` — a shielded-actor constant shared beyond the player.)
...
- **Rarity tiers & run-buff catalogue** (`14`): first-pass `RARITY_TIERS` (0.2) and `RUN_BUFFS` families/caps (0.3) shipped — all five families now real: `crit_chance` (`ENGINE_VERSION` 26, `07`'s crit sketch) and `flat_energy` (`ENGINE_VERSION` 60, `03`'s ammo economy — the one family a floor drop can never roll, see the note above). *Remaining:* the final base-quality numbers, and the CHOICE half of the offering flow. The shop route shipped 2026-09-14 (`ENGINE_VERSION` 64) — a buff is one of every counter's three fixed lines — so a buff is no longer only a table weight; what is still open (`ROADMAP` B2) is whether a counter or a chest hands over a pick-one-of-three the way the floor cards do, rather than a single line at a fixed price.
...
- **Character balance-test suite** (`14`): ✅ done (ROADMAP 2.3). A side-grade / no-all-rounder STUB shipped first (`skins.test.ts`, no Pareto domination on `(maxHp, maxShield)`, ROADMAP 0.5); the full suite covering all 3 launch characters — Pareto-non-domination, per-axis spread, equal-worth budget band, no inert passive on a zero-shield body — shipped in Phase 2.3. `ENGINE_VERSION` 60 widened Pareto and the spread check to the `(maxHp, maxShield, maxEnergy)` TRIPLE (stricter: one more column a would-be all-rounder must lose on) while leaving the budget band summing only the two defensive axes, and added a gate pinning the DIRECTION of the new axis by name — deepest pool on the smallest body. *Remaining:* extending it to future purchased characters as they're added.
