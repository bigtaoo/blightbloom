...
| WeChat mini-game | `npm run build:wechat` | bundles the IIFE, mirrors `public/` in by package and generates `game.json` into `../platforms/wechat/`; open that in WeChat DevTools. Real art loads, the game boots and renders, menu taps work, and the menus fit the 844x390 landscape-phone viewport (all 2026-08-25); the main package is `js/game.js` alone since the 2026-09-01 phase split (1.04 MB / 4.00 MB + 8 subpackages — run `npm run check:wechatpackage --verbose` rather than quoting these), and the target reaches the network since 2026-09-09 (`platform/wechat/weChatFetch.ts` + `weChatStorage.ts`). Real-device verification is what remains: frame rate, touch feel, lowest base-library version, and now one request actually leaving a handset — **build with `VITE_MATCHSVC_URL=https://…`, or `wx.request` refuses the localhost default and nothing says so.** See `../design/04-wechat.md` |
