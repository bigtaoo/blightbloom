/**
 * German translation (design/17-i18n.md). Must mirror `en.ts`'s exact key shape —
 * `Translations<typeof en>` (../index.ts) makes a missing/extra key a compile error.
 */
import type { Translations } from '..';
import type { en } from './en';

export const de: Translations<typeof en> = {
  mainMenu: {
    title: 'BLIGHTBLOOM',
    subtitle: 'hinabsteigen, bergen, überleben',
    play: 'SPIELEN',
    continueRun: 'FORTSETZEN ▸',
    continueRunAt: 'Etage {floor} · {m}:{ss} gespielt',
    solo: 'SOLO PvE',
    coop: 'KOOP',
    pvpSolo: 'PVP SOLO-QUEUE',
    tutorial: 'TUTORIAL',
    recommended: 'NEU HIER?',
    squad: 'TRUPP',
    forge: 'SCHMIEDE',
    account: 'ANMELDEN',
    settings: 'OPTIONEN',
    greeting: 'Hallo, {username}',
  },
  loading: {
    boot: 'LÄDT',
    art: 'LADE GRAFIKEN',
    enteringRun: 'BETRETE DEN DUNGEON',
    returningToHub: 'ZURÜCK ZUR LOBBY',
  },
  pauseMenu: {
    title: 'PAUSIERT',
    resume: 'FORTSETZEN',
    settings: 'EINSTELLUNGEN',
    saveQuit: 'SPEICHERN & ENDE',
    quit: 'ZUR SCHMIEDE',
  },
  settings: {
    title: 'EINSTELLUNGEN',
    master: 'Gesamt',
    sfx: 'Effekte',
    music: 'Musik',
    mute: 'STUMM',
    unmute: 'TON AN',
    language: 'SPRACHE: {name}',
    controlLayout: 'STEUERUNG: {mode}',
    controlLayoutStandard: 'STANDARD',
    controlLayoutMirrored: 'LINKSHÄNDIG',
    quality: 'QUALITÄT: {mode}',
    qualityAuto: 'AUTO',
    qualityHigh: 'HOCH',
    qualityLow: 'NIEDRIG',
    qualityAutoLow: 'AUTO (NIEDRIG)',
    qualityMedium: 'MITTEL',
    qualityAutoMedium: 'AUTO (MITTEL)',
    frameRate: 'BILDRATE: {fps}',
    reduceMotion: 'BEWEGUNG REDUZIEREN: {mode}',
    on: 'AN',
    off: 'AUS',
    tutorial: 'TUTORIAL WIEDERHOLEN',
    back: 'ZURÜCK',
  },
  forge: {
    title: 'SCHMIEDE-AUSSENPOSTEN',
    hint: '[↑↓] blättern · [1-9] schmieden · [B] Shop',
    backButton: '← ZURÜCK',
    pagePrevButton: '‹ SEITE',
    pageNextButton: 'SEITE ›',
    noneStarterPair: '(keine → {weapons})',
    moreAvailable: '{count} weitere verfügbar',
    materialsLine: 'Materialien   {bank}',
    loadoutLine: 'Ausrüstung  {loadout}   ({count}/{max})',
    storeLine: 'Shop: {items}  — [B] Shop öffnen',
    storeButton: 'SHOP',
    lockedFind: 'gesperrt (erst finden)',
    lockedSource: 'gesperrt ({source})',
    craftable: 'herstellbar',
    craftableSchematic: 'herstellbar (Bauplan ×{count})',
    needMaterials: 'Materialien fehlen',
    kindTaken: 'Waffenart belegt',
    stagedTag: '  ▸ausgerüstet×{count}',
    pageLabel: 'Seite {current}/{total}',
    equippedHeader: 'Ausgerüstet: {id}',
    candidateHeader: 'Auswahl: {id}',
  },
  loadout: {
    title: 'AUSRÜSTUNG',
    hint: '[C] Charakter · [X] leeren · [F] Schmiede · [Enter] Start',
    backButton: '← MENÜ',
    charStats: '{hp} LP / {sh} Schild',
    ownedChars: 'eigene Charaktere: {count}',
    materialsLine: 'Materialien   {bank}',
    slotsLine: 'Mitgeführte Waffen   {count}/{max}',
    craftedTag: 'geschmiedet',
    starterTag: 'Standardset',
    forgeCardName: 'SCHMIEDE',
    forgeCardHint: 'Waffen schmieden ▸',
    clearLoadout: 'AUSRÜSTUNG LEEREN',
    startRun: 'LAUF STARTEN ▸',
    continueRun: 'FORTSETZEN ▸',
    savedRunLine: 'Gespeicherter Lauf: Etage {floor}, {m}:{ss} gespielt — FORTSETZEN macht weiter, LAUF STARTEN verwirft ihn',
  },
  store: {
    title: 'SHOP',
    back: 'SCHMIEDE',
    loading: 'Shop wird geladen …',
    pickOne: 'Zum Kaufen auf einen Eintrag tippen. Die Preise kommen vom Server.',
    empty: 'Derzeit ist nichts im Angebot.',
    row: '{item}   {price}',
    rowOwned: '{item}   IM BESITZ',
    pagePrev: '‹ SEITE',
    pageNext: 'SEITE ›',
    pageLabel: 'Seite {current}/{total}',
    purchasing: '{item} wird gekauft – warte auf die Zahlung …',
    purchased: '{item} gehört dir. Es liegt jetzt in der Schmiede.',
    purchasedNoRefresh: '{item} gehört dir. Es erscheint bei der nächsten Anmeldung in der Schmiede.',
    alreadyOwned: 'Du besitzt {item} bereits.',
    busy: 'Eins nach dem anderen – die letzte Anfrage läuft noch.',
    loginRequired: 'Melde dich zuerst an. Käufe gehören zu deinem Konto, nicht zu diesem Gerät.',
    unavailableHere: 'In dieser Version des Spiels sind Käufe nicht verfügbar.',
    notConfigured: 'Käufe sind derzeit nicht möglich: In dieser Version ist kein Zahlungsanbieter eingerichtet. Es wurde nichts abgebucht.',
    orderFailed: 'Der Kauf konnte nicht gestartet werden. Es wurde nichts abgebucht.',
    paymentFailed: 'Die Zahlung ist fehlgeschlagen. Es wurde nichts abgebucht.',
    pending: 'Warte weiterhin auf die Zahlung. Geht sie durch, erscheint der Gegenstand bei der nächsten Anmeldung.',
    listFailed: 'Der Shop konnte nicht geladen werden. Prüfe deine Verbindung und versuche es erneut.',
  },
  hud: {
    pickupPrompt: { title: 'Waffen in der Nähe' },
    shop: {
      title: 'Laden  {coins}',
      price: '{price}',
      sold: 'VERKAUFT',
      buffPick: 'Einen Buff wählen',
      heal: 'Heiltrank',
      energy: 'Energiezelle',
      shield: 'Schildbatterie',
      emp: 'EMP-Granate',
    },
    chest: {
      bigTitle: 'Große Truhe',
      plates: 'Platten besetzt {on}/{total}',
    },
    chips: {
      floor: 'ETAGE',
      room: 'RAUM',
      enemies: 'GEGNER',
      banked: 'GESICHERT',
      coins: 'MÜNZEN',
      score: 'PUNKTE',
      buffs: 'BUFFS',
      alive: 'ÜBRIG',
      stage: 'ZONE',
    },
    weapon: {
      none: 'KEINE WAFFE',
      unarmed: 'unbewaffnet',
      sub: '{rarity} · {kind} · {element}',
      damage: 'SCHADEN {damage}',
    },
    rarity: {
      common: 'gewöhnlich',
      fine: 'fein',
      epic: 'episch',
      legend: 'legendär',
      legendary: 'uralt',
    },
    kind: {
      ranged: 'Fernkampf',
      melee: 'Nahkampf',
    },
    element: {
      physical: 'physisch',
      fire: 'Feuer',
      ice: 'Eis',
      lightning: 'Blitz',
      poison: 'Gift',
    },
    elementShort: {
      physical: 'PHY',
      fire: 'FEU',
      ice: 'EIS',
      lightning: 'BLI',
      poison: 'GFT',
    },
    source: {
      purchase: 'Kauf',
      event: 'Event',
    },
    ally: {
      tag: 'VERBÜNDETER · {skin}',
      downed: 'AM BODEN {seconds}s',
      reviving: 'WIEDERBELEBUNG {pct}%',
    },
    downed: {
      title: 'DU BIST AM BODEN',
      bleedout: 'Verblutet in {seconds}s',
      reviving: 'WIRD WIEDERBELEBT…',
    },
    roster: { you: 'DU' },
    portalTitle: 'ETAGE GESÄUBERT — ein Portal hat sich geöffnet',
    portalExtract: 'Sichern & Extrahieren ({pending} Materialien)',
    portalDescend: 'Zu Etage {floor} absteigen',
    portalTitleBoss: 'BOSS BESIEGT — das Extraktionsportal ist offen',
    floorCardTitle: 'Verbesserung wählen',
    floorCardVotes: '{n} Stimmen',
  },
  auth: {
    title: 'KONTO',
    login: 'ANMELDEN',
    register: 'REGISTRIEREN',
    changePassword: 'PASSWORT ÄNDERN',
    logout: 'ABMELDEN',
    back: 'ZURÜCK',
    usernamePlaceholder: 'BENUTZERNAME',
    usernameRequired: 'Benutzername erforderlich.',
    passwordPlaceholder: 'PASSWORT',
    currentPasswordPlaceholder: 'AKTUELLES PASSWORT',
    newPasswordPlaceholder: 'NEUES PASSWORT',
    loginFailed: 'Anmeldung fehlgeschlagen — bitte erneut versuchen.',
    registerFailed: 'Registrierung fehlgeschlagen — bitte erneut versuchen.',
    throttled: 'Zu viele Versuche — bitte in ein paar Minuten erneut versuchen.',
    passwordChanged: 'Passwort geändert.',
    passwordChangeFailed: 'Passwort konnte nicht geändert werden.',
    loggedInAs: 'Angemeldet als {username}',
    playingAsGuest: 'Als Gast spielen',
    dataNotice:
      'Optional. Bei der Registrierung speichern wir Benutzername, Passwort-Hash und Fortschritt auf unserem Server. Ohne Konto wird nichts gesendet.',
    portalDataNotice: 'Solange du bei CrazyGames angemeldet bist, speichern wir deinen CrazyGames-Namen und deinen Spielfortschritt auf unserem Server.',
    privacyLink: 'Datenschutzerklärung',
    mergeTitle: 'GASTFORTSCHRITT ÜBERNEHMEN?',
    mergeBody:
      'Auf diesem Gerät liegt Gastfortschritt, den {username} nicht hat: {materials} Materialien, {blueprints} Baupläne, {characters} Charaktere. Diese Frage kommt einmal pro Gerät.',
    mergeUseAccount: 'KONTO VERWENDEN',
    mergeCombine: 'BEIDES ZUSAMMEN',
    sessionExpiredTitle: 'ABGEMELDET',
    sessionExpiredBody:
      'Deine gespeicherte Sitzung ist nicht mehr gültig, du spielst jetzt als Gast. Der Fortschritt auf diesem Gerät bleibt unangetastet — melde dich erneut an, um ihn zu synchronisieren.',
    noticeDismiss: 'OK',
  },
  party: {
    title: 'TRUPP',
    create: 'TRUPP ERSTELLEN',
    createCoop: 'KOOP-GRUPPE ERSTELLEN',
    join: 'CODE BEITRETEN',
    startMatching: 'SUCHE STARTEN',
    leave: 'TRUPP VERLASSEN',
    back: 'ZURÜCK',
    partyClosed: 'Trupp aufgelöst.',
    createFailed: 'Trupp konnte nicht erstellt werden — bitte erneut versuchen.',
    createThrottled: 'Zu viele Trupps erstellt — bitte in ein paar Minuten erneut versuchen.',
    codePlaceholder: 'CODE',
    invalidCode: 'Code ungültig oder Trupp voll.',
    joinThrottled: 'Zu viele Versuche — bitte in ein paar Minuten erneut versuchen.',
    startFailed: 'Suche konnte nicht gestartet werden — bitte erneut versuchen.',
    codeLine: 'CODE: {code}',
    you: 'du',
    headerCoop: 'KOOP · {count}/{capacity}',
    headerSquad: 'PVP-TRUPP · {count}/{capacity}',
  },
  pvpPreview: {
    title: 'PVP-MATCH',
    map: 'Karte: {name} ({rooms} Räume)',
    fairnessNote: 'Werte ×{factor} für PvP skaliert — dauerhafte Ausrüstung wird nie mitgenommen.',
    queue: 'SUCHEN',
    back: 'ZURÜCK',
  },
  matchmaking: {
    searching: 'Suche nach einem Match…',
    connecting: 'Verbindung wird hergestellt…',
    elapsed: '{seconds}s vergangen',
    botSoon: 'KI-Spieler füllen freie Plätze in {seconds}s',
    botNow: 'Freie Plätze werden mit KI-Spielern gefüllt…',
    partyMembers: '{count} im Trupp',
    cancel: 'ABBRECHEN',
    errorTitle: 'MATCH FEHLGESCHLAGEN',
    retry: 'ERNEUT VERSUCHEN',
    back: 'ZURÜCK',
    errorTimeout: 'Zeitüberschreitung bei der Suche nach einem Match.',
    errorCancelled: 'Suche abgebrochen.',
    errorGeneric: 'Verbindung fehlgeschlagen — bitte erneut versuchen.',
    errorThrottled: 'Zu viele Versuche — warte ein paar Minuten vor der nächsten Suche.',
  },
  tutorial: {
    hintMoveKeys: 'Mit WASD bewegen. Linke Maustaste halten, um anzugreifen — das Zielen läuft automatisch.',
    hintMoveTouch: 'Zum Bewegen die linke Bildschirmhälfte ziehen, zum Angreifen die rechte halten — das Zielen läuft automatisch.',
    hintSwapKeys: 'Drücke 1 oder 2, um die Waffe zu wechseln.',
    hintSwapTouch: 'Tippe auf eine Waffentaste, um die Waffe zu wechseln.',
    hintDeflect: 'Schlage mit deiner Nahkampfwaffe in ankommende Kugeln, um sie abzuwehren.',
    hintCleared: 'Gut gemacht — auf zum Portal.',
    skip: 'TUTORIAL ÜBERSPRINGEN',
  },
  results: {
    mainMenuButton: 'HAUPTMENÜ',
    extractedTitle: 'EXTRAHIERT',
    defeatTitle: 'NIEDERLAGE',
    victoryTitle: 'SIEG ROYALE',
    eliminatedTitle: 'AUSGESCHIEDEN',
    confirmButton: 'BESTÄTIGEN',
    timeLine: 'Zeit {m}:{ss}',
    floorLine: 'Etage {floor}/{floorCount}',
    materialsBanked: 'Materialien gesichert: {count}',
    doubleMaterialsButton: 'WERBUNG ANSEHEN: MATERIAL x2',
    materialsDoubled: 'Materialien gesichert: {count} (Werbebonus x2)',
    adNotFilled: 'Keine Werbung verfügbar – deine {count} Materialien sind sicher',
    scoreLine: 'Punkte {score}',
    blueprintLine: 'Blaupause erbeutet: {weapon}',
    characterLine: 'Charakter freigeschaltet: {character}',
    fellOnFloor: 'Gefallen auf Etage {floor}/{floorCount}',
    materialsLost: 'Alle {count} mitgeführten Materialien sind verloren',
    placeOf: '1. Platz von {total}',
    placedOfTotal: 'Platz {place}/{total}',
    guestNotRanked: 'Als Gast nicht gewertet. Anmelden zum Werten.',
    connectionLostTitle: 'VERBINDUNG VERLOREN',
    connectionLostBody: 'Verbindung zum Match konnte nicht wiederhergestellt werden.',
  },
  toast: {
    heal: '+1 LP',
    shield: 'Schild aufgeladen',
    emp: 'EMP-Explosion',
    newWeapon: 'Neue Waffe',
    schematicFound: 'Bauplan: {weapon}',
    characterFound: 'Neuer Charakter: {character}',
    buffNamed: 'Buff: {id}',
    buffGeneric: 'Buff',
    materialQty: '+{qty} {material}',
    materialFallback: 'Material',
    reconnecting: 'Verbinde erneut…',
    reconnected: 'Wieder verbunden',
    replaySaved: 'Wiederholung gespeichert: {name}',
    replayNoRun: 'Noch nichts zu speichern',
    replayUnsupported: 'Dieses Gerät kann keine Dateien speichern',
    runSaved: 'Lauf gespeichert — in der Schmiede fortsetzen',
    runSaveFailed: 'Dieser Lauf konnte nicht gespeichert werden',
    runSaveOldVersion: 'Dieser gespeicherte Lauf ist von einer älteren Spielversion',
    runSaveStale: 'Dieser gespeicherte Lauf passt nicht mehr zu diesem Verlies',
  },
  compareCard: {
    damage: 'Schaden',
    type: 'Typ',
    fireRate: 'Feuerrate',
    spread: 'Streuung',
    speed: 'Geschwindigkeit',
    swing: 'Schwung',
    arc: 'Bogen',
    reach: 'Reichweite',
    deflect: 'Abwehr',
    yes: 'ja',
    no: 'nein',
    vs: 'ggü.',
  },
  weapon: {
    blaster: { name: 'Blaster' },
    saber: { name: 'Säbel' },
    repeater: { name: 'Repetierer' },
    cannon: { name: 'Kanone' },
    enemygun: { name: 'Plünderergewehr' },
    enemyclaw: { name: 'Räuberklaue' },
    enemymaul: { name: 'Räuberhammer' },
    enemynova: { name: 'Räubernova' },
    flamer: { name: 'Flammenwerfer' },
    cryobolt: { name: 'Kryobolzen' },
    teslagun: { name: 'Teslakanone' },
    venomspit: { name: 'Giftspucker' },
    emberblade: { name: 'Glutklinge' },
    frostbrand: { name: 'Frostklinge' },
    stormglaive: { name: 'Sturmglefe' },
    scattergun: { name: 'Streuflinte' },
    seeker: { name: 'Sucher' },
    mortar: { name: 'Mörser' },
    lasercutter: { name: 'Laserschneider' },
    tomahawk: { name: 'Tomahawk' },
    hammer: { name: 'Hammer' },
    spear: { name: 'Speer' },
    novaburst: { name: 'Novastoß' },
    gyre: { name: 'Strudel' },
    carom: { name: 'Abpraller' },
    leech: { name: 'Blutsauger' },
    cinderscatter: { name: 'Glutstreuer' },
    frostseeker: { name: 'Frostsucher' },
  },
  skin: {
    vanguard: { name: 'Vorhut' },
    skirmisher: { name: 'Plänkler' },
    juggernaut: { name: 'Juggernaut' },
  },
  card: {
    potion_flow: { name: 'Lebensquell', desc: 'Tränke fallen {factor}x so oft' },
    windfall: { name: 'Geldsegen', desc: 'Münzen sind {factor}x wert' },
    edge: { name: 'Schärfe', desc: '+{value}% Schaden' },
    cadence: { name: 'Kadenz', desc: '+{value}% Feuerrate' },
    bulwark: { name: 'Bollwerk', desc: '+{value} max. LP' },
    precision: { name: 'Präzision', desc: '+{value}% Kritische Chance' },
    capacitor: { name: 'Kondensator', desc: '+{value} max. Energie' },
    surge: { name: 'Schub', desc: 'Energie ist {factor}x wert' },
    aegis: { name: 'Ägide', desc: 'Schilde sind {factor}x wert' },
    stockpile: { name: 'Vorrat', desc: 'Material ist {factor}x wert' },
    bounty: { name: 'Beute', desc: '+{count} Waffe pro Kiste' },
  },
  buff: {
    dmg_up: { name: 'Schaden erhöht' },
    rof_up: { name: 'Feuerrate erhöht' },
    vit_up: { name: 'Vitalität erhöht' },
    crit_up: { name: 'Krit.-Chance erhöht' },
    cell_up: { name: 'Max. Energie erhöht' },
  },
  material: {
    physical: { name: 'Physisch' },
    fire: { name: 'Feuer' },
    ice: { name: 'Eis' },
    lightning: { name: 'Blitz' },
    poison: { name: 'Gift' },
  },
};
