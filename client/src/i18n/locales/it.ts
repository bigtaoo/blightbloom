/**
 * Italian translation (design/17-i18n.md). Must mirror `en.ts`'s exact key shape —
 * `Translations<typeof en>` (../index.ts) makes a missing/extra key a compile error.
 */
import type { Translations } from '..';
import type { en } from './en';

export const it: Translations<typeof en> = {
  mainMenu: {
    title: 'BLIGHTBLOOM',
    subtitle: 'scendi, estrai, sopravvivi',
    play: 'GIOCA',
    continueRun: 'CONTINUA ▸',
    continueRunAt: 'Piano {floor} · {m}:{ss} giocati',
    solo: 'SOLO PvE',
    coop: 'COOP',
    pvpSolo: 'CODA PVP SOLO',
    tutorial: 'TUTORIAL',
    recommended: 'NUOVO QUI?',
    squad: 'SQUADRA',
    forge: 'FUCINA',
    account: 'ACCEDI',
    settings: 'OPZIONI',
    greeting: 'Ciao, {username}',
  },
  loading: {
    boot: 'CARICAMENTO',
    art: 'CARICAMENTO GRAFICA',
    enteringRun: 'INGRESSO NEL DUNGEON',
    returningToHub: 'RITORNO ALLA LOBBY',
  },
  pauseMenu: {
    title: 'IN PAUSA',
    resume: 'RIPRENDI',
    settings: 'IMPOSTAZIONI',
    saveQuit: 'SALVA ED ESCI',
    quit: 'TORNA ALLA FUCINA',
  },
  settings: {
    title: 'IMPOSTAZIONI',
    master: 'Generale',
    sfx: 'Effetti',
    music: 'Musica',
    mute: 'MUTO',
    unmute: 'ATTIVA AUDIO',
    language: 'LINGUA: {name}',
    controlLayout: 'COMANDI: {mode}',
    controlLayoutStandard: 'STANDARD',
    controlLayoutMirrored: 'MANCINO',
    quality: 'QUALITÀ: {mode}',
    qualityAuto: 'AUTO',
    qualityHigh: 'ALTA',
    qualityLow: 'BASSA',
    qualityAutoLow: 'AUTO (BASSA)',
    qualityMedium: 'MEDIA',
    qualityAutoMedium: 'AUTO (MEDIA)',
    frameRate: 'FRAME RATE: {fps}',
    reduceMotion: 'RIDUCI MOVIMENTO: {mode}',
    on: 'SÌ',
    off: 'NO',
    tutorial: 'RIPETI TUTORIAL',
    back: 'INDIETRO',
  },
  forge: {
    title: 'AVAMPOSTO DELLA FUCINA',
    hint: '[↑↓] sfoglia · [1-9] forgia · [B] negozio',
    backButton: '← INDIETRO',
    pagePrevButton: '‹ PAGINA',
    pageNextButton: 'PAGINA ›',
    noneStarterPair: '(nessuna → {weapons})',
    moreAvailable: 'altri disponibili: {count}',
    materialsLine: 'Materiali   {bank}',
    loadoutLine: 'Equipaggiamento  {loadout}   ({count}/{max})',
    storeLine: 'Negozio: {items}  — [B] apri il negozio',
    storeButton: 'NEGOZIO',
    lockedFind: 'bloccato (da trovare)',
    lockedSource: 'bloccato ({source})',
    craftable: 'forgiabile',
    craftableSchematic: 'forgiabile (schema ×{count})',
    needMaterials: 'materiali insufficienti',
    kindTaken: 'tipo già occupato',
    stagedTag: '  ▸equipaggiato×{count}',
    pageLabel: 'Pagina {current}/{total}',
    equippedHeader: 'Equipaggiato: {id}',
    candidateHeader: 'Candidato: {id}',
  },
  loadout: {
    title: 'EQUIPAGGIAMENTO',
    hint: '[C] personaggio · [X] svuota · [F] fucina · [Invio] inizia',
    backButton: '← MENU',
    charStats: '{hp} PV / {sh} Scudo',
    ownedChars: 'personaggi posseduti: {count}',
    materialsLine: 'Materiali   {bank}',
    slotsLine: 'Armi portate   {count}/{max}',
    craftedTag: 'forgiata',
    starterTag: 'kit predefinito',
    forgeCardName: 'FUCINA',
    forgeCardHint: 'forgia armi ▸',
    clearLoadout: 'SVUOTA',
    startRun: 'INIZIA PARTITA ▸',
    continueRun: 'CONTINUA ▸',
    savedRunLine: 'Partita salvata: piano {floor}, {m}:{ss} giocati — CONTINUA per riprendere, INIZIA PARTITA la scarta',
  },
  store: {
    title: 'NEGOZIO',
    back: 'FUCINA',
    loading: 'Caricamento del negozio…',
    pickOne: 'Tocca un articolo per acquistarlo. I prezzi arrivano dal server.',
    empty: 'Al momento non c’è nulla in vendita.',
    row: '{item}   {price}',
    rowOwned: '{item}   POSSEDUTO',
    pagePrev: '‹ PAGINA',
    pageNext: 'PAGINA ›',
    pageLabel: 'Pagina {current}/{total}',
    purchasing: 'Acquisto di {item} — in attesa del pagamento…',
    purchased: '{item} è tuo. Ora si trova nella fucina.',
    purchasedNoRefresh: '{item} è tuo. Comparirà nella fucina al prossimo accesso.',
    alreadyOwned: 'Possiedi già {item}.',
    busy: 'Una cosa alla volta: la richiesta precedente è ancora in corso.',
    loginRequired: 'Accedi prima. Gli acquisti appartengono al tuo account, non a questo dispositivo.',
    unavailableHere: 'Gli acquisti non sono disponibili in questa versione del gioco.',
    notConfigured: 'Acquisti non disponibili al momento: questa versione non ha un fornitore di pagamento configurato. Non è stato addebitato nulla.',
    orderFailed: 'Impossibile avviare l’acquisto. Non è stato addebitato nulla.',
    paymentFailed: 'Il pagamento non è andato a buon fine. Non è stato addebitato nulla.',
    pending: 'Pagamento ancora in attesa. Se andrà a buon fine, l’oggetto comparirà al prossimo accesso.',
    listFailed: 'Impossibile caricare il negozio. Controlla la connessione e riprova.',
  },
  hud: {
    pickupPrompt: { title: 'Armi vicine' },
    shop: {
      title: 'Negozio  {coins}',
      price: '{price}',
      sold: 'VENDUTO',
      buffPick: 'Scegli un potenziamento',
      heal: 'Pozione curativa',
      energy: 'Cella energetica',
      shield: 'Batteria scudo',
      emp: 'Granata EMP',
    },
    chest: {
      bigTitle: 'Forziere grande',
      plates: 'Piastre occupate {on}/{total}',
    },
    chips: {
      floor: 'PIANO',
      room: 'STANZA',
      enemies: 'NEMICI',
      banked: 'DEPOSITATO',
      coins: 'MONETE',
      score: 'PUNTI',
      buffs: 'BONUS',
      alive: 'VIVI',
      stage: 'ZONA',
    },
    weapon: {
      none: 'NESSUNA ARMA',
      unarmed: 'disarmato',
      sub: '{rarity} · {kind} · {element}',
      damage: 'DANNO {damage}',
    },
    rarity: {
      common: 'comune',
      fine: 'raffinata',
      epic: 'epica',
      legend: 'leggendaria',
      legendary: 'ancestrale',
    },
    kind: {
      ranged: 'a distanza',
      melee: 'da mischia',
    },
    element: {
      physical: 'fisico',
      fire: 'fuoco',
      ice: 'ghiaccio',
      lightning: 'fulmine',
      poison: 'veleno',
    },
    elementShort: {
      physical: 'FIS',
      fire: 'FUO',
      ice: 'GHI',
      lightning: 'FUL',
      poison: 'VEL',
    },
    source: {
      purchase: 'acquisto',
      event: 'evento',
    },
    ally: {
      tag: 'ALLEATO · {skin}',
      downed: 'A TERRA {seconds}s',
      reviving: 'RIANIMAZIONE {pct}%',
    },
    downed: {
      title: 'SEI A TERRA',
      bleedout: 'Dissanguamento — {seconds}s rimasti',
      reviving: 'RIANIMAZIONE IN CORSO…',
    },
    roster: { you: 'TU' },
    portalTitle: 'PIANO RIPULITO — un portale si è aperto',
    portalExtract: 'Deposita ed estrai ({pending} materiali)',
    portalDescend: 'Scendi al piano {floor}',
    portalTitleBoss: 'BOSS SCONFITTO — il portale di estrazione è aperto',
    floorCardTitle: 'Scegli un potenziamento',
    floorCardVotes: '{n} voti',
  },
  auth: {
    title: 'ACCOUNT',
    login: 'ACCEDI',
    register: 'REGISTRATI',
    changePassword: 'CAMBIA PASSWORD',
    logout: 'ESCI',
    back: 'INDIETRO',
    usernamePlaceholder: 'NOME UTENTE',
    usernameRequired: 'Nome utente richiesto.',
    passwordPlaceholder: 'PASSWORD',
    currentPasswordPlaceholder: 'PASSWORD ATTUALE',
    newPasswordPlaceholder: 'NUOVA PASSWORD',
    loginFailed: 'Accesso non riuscito — riprova.',
    registerFailed: 'Registrazione non riuscita — riprova.',
    throttled: 'Troppi tentativi — riprova tra qualche minuto.',
    passwordChanged: 'Password cambiata.',
    passwordChangeFailed: 'Impossibile cambiare la password.',
    loggedInAs: 'Accesso effettuato come {username}',
    playingAsGuest: 'Gioco come ospite',
    dataNotice:
      'Opzionale. La registrazione salva nome utente, hash della password e progressi sul nostro server. Senza account non viene inviato nulla.',
    portalDataNotice: 'Quando hai effettuato l\'accesso su CrazyGames, conserviamo il tuo nome CrazyGames e i tuoi progressi sul nostro server.',
    privacyLink: 'Informativa sulla privacy',
    mergeTitle: 'PORTARE I PROGRESSI OSPITE?',
    mergeBody:
      'Questo dispositivo ha progressi ospite che {username} non ha: {materials} materiali, {blueprints} progetti, {characters} personaggi. La domanda viene fatta una volta per dispositivo.',
    mergeUseAccount: "USA L'ACCOUNT",
    mergeCombine: 'UNISCI ENTRAMBI',
    sessionExpiredTitle: 'DISCONNESSO',
    sessionExpiredBody:
      'La sessione salvata non è più valida, quindi stai giocando come ospite. I progressi su questo dispositivo sono intatti: accedi di nuovo per sincronizzarli.',
    noticeDismiss: 'OK',
  },
  party: {
    title: 'SQUADRA',
    create: 'CREA SQUADRA',
    createCoop: 'CREA GRUPPO CO-OP',
    join: 'ENTRA CON CODICE',
    startMatching: 'AVVIA RICERCA',
    leave: 'LASCIA LA SQUADRA',
    back: 'INDIETRO',
    partyClosed: 'Squadra sciolta.',
    createFailed: 'Impossibile creare la squadra — riprova.',
    createThrottled: 'Troppe squadre create — riprova tra qualche minuto.',
    codePlaceholder: 'CODICE',
    invalidCode: 'Codice non valido o squadra al completo.',
    joinThrottled: 'Troppi tentativi — riprova tra qualche minuto.',
    startFailed: 'Impossibile avviare la ricerca — riprova.',
    codeLine: 'CODICE: {code}',
    you: 'tu',
    headerCoop: 'CO-OP · {count}/{capacity}',
    headerSquad: 'SQUADRA PVP · {count}/{capacity}',
  },
  pvpPreview: {
    title: 'PARTITA PVP',
    map: 'Mappa: {name} ({rooms} stanze)',
    fairnessNote: 'Statistiche scalate ×{factor} per il PvP — l’equipaggiamento permanente non viene mai portato.',
    queue: 'CERCA',
    back: 'INDIETRO',
  },
  matchmaking: {
    searching: 'Ricerca di una partita…',
    connecting: 'Connessione…',
    elapsed: '{seconds}s trascorsi',
    botSoon: 'Giocatori IA riempiono i posti liberi tra {seconds}s',
    botNow: 'Riempimento dei posti liberi con giocatori IA…',
    partyMembers: '{count} in squadra',
    cancel: 'ANNULLA',
    errorTitle: 'PARTITA FALLITA',
    retry: 'RIPROVA',
    back: 'INDIETRO',
    errorTimeout: 'Tempo scaduto in attesa di una partita.',
    errorCancelled: 'Ricerca annullata.',
    errorGeneric: 'Impossibile connettersi — riprova.',
    errorThrottled: 'Troppi tentativi — attendi qualche minuto prima di cercare di nuovo.',
  },
  tutorial: {
    hintMoveKeys: 'Muoviti con WASD. Tieni premuto il tasto sinistro del mouse per attaccare: la mira è automatica.',
    hintMoveTouch: 'Trascina il lato sinistro dello schermo per muoverti e tieni premuto il lato destro per attaccare: la mira è automatica.',
    hintSwapKeys: 'Premi 1 o 2 per cambiare arma.',
    hintSwapTouch: 'Tocca un pulsante arma per cambiare arma.',
    hintDeflect: 'Colpisci i proiettili in arrivo con la tua arma da mischia per deviarli.',
    hintCleared: 'Ben fatto — dirigiti al portale.',
    skip: 'SALTA TUTORIAL',
  },
  results: {
    mainMenuButton: 'MENU PRINCIPALE',
    extractedTitle: 'ESTRAZIONE RIUSCITA',
    defeatTitle: 'SCONFITTA',
    victoryTitle: 'VITTORIA ROYALE',
    eliminatedTitle: 'ELIMINATO',
    confirmButton: 'CONFERMA',
    timeLine: 'Tempo {m}:{ss}',
    floorLine: 'Piano {floor}/{floorCount}',
    materialsBanked: 'Materiali depositati: {count}',
    doubleMaterialsButton: 'GUARDA UN ANNUNCIO: MATERIALI x2',
    materialsDoubled: 'Materiali depositati: {count} (bonus annuncio x2)',
    adNotFilled: 'Nessun annuncio disponibile: i tuoi {count} materiali sono al sicuro',
    scoreLine: 'Punteggio {score}',
    blueprintLine: 'Progetto recuperato: {weapon}',
    characterLine: 'Personaggio sbloccato: {character}',
    fellOnFloor: 'Caduto al piano {floor}/{floorCount}',
    materialsLost: 'Tutti i {count} materiali trasportati sono andati persi',
    placeOf: '1° posto su {total}',
    placedOfTotal: 'Posizione {place}/{total}',
    guestNotRanked: 'Ospite: non in classifica. Accedi per entrarci.',
    connectionLostTitle: 'CONNESSIONE PERSA',
    connectionLostBody: 'Impossibile riconnettersi alla partita.',
  },
  toast: {
    heal: '+1 PV',
    shield: 'Scudo ricaricato',
    emp: 'Esplosione EMP',
    newWeapon: 'Nuova arma',
    schematicFound: 'Schema: {weapon}',
    characterFound: 'Nuovo personaggio: {character}',
    buffNamed: 'Bonus: {id}',
    buffGeneric: 'Bonus',
    materialQty: '+{qty} {material}',
    materialFallback: 'materiale',
    reconnecting: 'Riconnessione…',
    reconnected: 'Riconnesso',
    replaySaved: 'Replay salvato: {name}',
    replayNoRun: 'Non c’è ancora nulla da salvare',
    replayUnsupported: 'Questo dispositivo non può salvare file',
    runSaved: 'Partita salvata — riprendila dalla fucina',
    runSaveFailed: 'Impossibile salvare questa partita',
    runSaveOldVersion: 'Quella partita salvata è di una versione precedente del gioco',
    runSaveStale: 'Quella partita salvata non corrisponde più a questo dungeon',
  },
  compareCard: {
    damage: 'Danno',
    type: 'Tipo',
    fireRate: 'Cadenza di tiro',
    spread: 'Dispersione',
    speed: 'Velocità',
    swing: 'Colpo',
    arc: 'Arco',
    reach: 'Portata',
    deflect: 'Deviazione',
    yes: 'sì',
    no: 'no',
    vs: 'vs',
  },
  weapon: {
    blaster: { name: 'Blaster' },
    saber: { name: 'Sciabola' },
    repeater: { name: 'Ripetitore' },
    cannon: { name: 'Cannone' },
    enemygun: { name: 'Fucile del predone' },
    enemyclaw: { name: 'Artiglio del predone' },
    enemymaul: { name: 'Maglio del predone' },
    enemynova: { name: 'Nova del predone' },
    flamer: { name: 'Lanciafiamme' },
    cryobolt: { name: 'Criobolt' },
    teslagun: { name: 'Cannone Tesla' },
    venomspit: { name: 'Sputaveleno' },
    emberblade: { name: 'Lama di braci' },
    frostbrand: { name: 'Lama di gelo' },
    stormglaive: { name: 'Falcione della tempesta' },
    scattergun: { name: 'Fucile a dispersione' },
    seeker: { name: 'Cercatore' },
    mortar: { name: 'Mortaio' },
    lasercutter: { name: 'Tagliatore laser' },
    tomahawk: { name: 'Tomahawk' },
    hammer: { name: 'Martello' },
    spear: { name: 'Lancia' },
    novaburst: { name: 'Esplosione nova' },
    gyre: { name: 'Vortice' },
    carom: { name: 'Rimbalzo' },
    leech: { name: 'Sanguisuga' },
    cinderscatter: { name: 'Dispersione di braci' },
    frostseeker: { name: 'Cercatore di gelo' },
  },
  skin: {
    vanguard: { name: 'Avanguardia' },
    skirmisher: { name: 'Schermagliatore' },
    juggernaut: { name: 'Colosso' },
  },
  card: {
    potion_flow: { name: 'Flusso vitale', desc: 'Pozioni {factor}x più frequenti' },
    windfall: { name: 'Fortuna', desc: 'Le monete valgono {factor}x' },
    edge: { name: 'Affilatura', desc: '+{value}% danni' },
    cadence: { name: 'Cadenza', desc: '+{value}% cadenza di fuoco' },
    bulwark: { name: 'Baluardo', desc: '+{value} PS massimi' },
    precision: { name: 'Precisione', desc: '+{value}% probabilità critica' },
    capacitor: { name: 'Condensatore', desc: '+{value} energia massima' },
    surge: { name: 'Sovraccarico', desc: 'L’energia vale {factor}x' },
    aegis: { name: 'Egida', desc: 'Gli scudi valgono {factor}x' },
    stockpile: { name: 'Scorta', desc: 'I materiali valgono {factor}x' },
    bounty: { name: 'Taglia', desc: '+{count} arma per forziere' },
  },
  buff: {
    dmg_up: { name: 'Danno aumentato' },
    rof_up: { name: 'Cadenza di tiro aumentata' },
    vit_up: { name: 'Vitalità aumentata' },
    crit_up: { name: 'Probabilità di critico aumentata' },
    cell_up: { name: 'Energia massima aumentata' },
  },
  material: {
    physical: { name: 'Fisico' },
    fire: { name: 'Fuoco' },
    ice: { name: 'Ghiaccio' },
    lightning: { name: 'Fulmine' },
    poison: { name: 'Veleno' },
  },
};
