/**
 * Chinese translation (design/17-i18n.md). Must mirror `en.ts`'s exact key shape —
 * `Translations<typeof en>` (../index.ts) makes a missing/extra key a compile error.
 */
import type { Translations } from '..';
import type { en } from './en';

export const zh: Translations<typeof en> = {
  mainMenu: {
    title: '绽晶',
    subtitle: '深入·撤离·生存',
    play: '开始',
    continueRun: '继续行动 ▸',
    continueRunAt: '第 {floor} 层 · 已进行 {m}:{ss}',
    solo: '单人闯关',
    coop: '联机合作',
    pvpSolo: '大逃杀单排',
    tutorial: '新手教程',
    recommended: '新手推荐',
    squad: '组队',
    forge: '锻造场',
    account: '登录',
    settings: '设置',
    greeting: '你好，{username}',
  },
  loading: {
    boot: '加载中',
    art: '正在加载素材',
    enteringRun: '正在进入地图',
    returningToHub: '正在返回大厅',
  },
  pauseMenu: {
    title: '已暂停',
    resume: '继续',
    settings: '设置',
    saveQuit: '保存并退出',
    quit: '返回锻造场',
  },
  settings: {
    title: '设置',
    master: '总音量',
    sfx: '音效',
    music: '音乐',
    mute: '静音',
    unmute: '取消静音',
    language: '语言：{name}',
    controlLayout: '操作布局：{mode}',
    controlLayoutStandard: '标准',
    controlLayoutMirrored: '左手模式',
    quality: '画质: {mode}',
    qualityAuto: '自动',
    qualityHigh: '高',
    qualityLow: '低',
    qualityAutoLow: '自动 (低)',
    qualityMedium: '中',
    qualityAutoMedium: '自动 (中)',
    frameRate: '帧率: {fps}',
    reduceMotion: '减少画面抖动: {mode}',
    on: '开',
    off: '关',
    tutorial: '重玩教程',
    back: '返回',
  },
  forge: {
    title: '锻造场',
    hint: '[↑↓] 浏览 · [1-9] 打造 · [B] 商店',
    backButton: '← 返回',
    pagePrevButton: '‹ 上一页',
    pageNextButton: '下一页 ›',
    noneStarterPair: '（无 → {weapons}）',
    moreAvailable: '还有 {count} 件可购买',
    materialsLine: '材料   {bank}',
    loadoutLine: '装备     {loadout}   （{count}/{max}）',
    storeLine: '商店：{items}  — [B] 打开商店',
    storeButton: '商店',
    lockedFind: '未解锁（需寻获）',
    lockedSource: '未解锁（{source}）',
    craftable: '可打造',
    craftableSchematic: '可打造（图纸 ×{count}）',
    needMaterials: '材料不足',
    kindTaken: '该类型已占用',
    stagedTag: '  ▸已装备×{count}',
    pageLabel: '第 {current}/{total} 页',
    equippedHeader: '当前装备：{id}',
    candidateHeader: '候选：{id}',
  },
  loadout: {
    title: '出击准备',
    hint: '[C] 切换角色 · [X] 清空装备 · [F] 锻造场 · [Enter] 出发',
    backButton: '← 菜单',
    charStats: '{hp} 生命 / {sh} 护盾',
    ownedChars: '已拥有角色：{count}',
    materialsLine: '材料   {bank}',
    slotsLine: '携带武器   {count}/{max}',
    craftedTag: '已锻造',
    starterTag: '默认武器',
    forgeCardName: '锻造场',
    forgeCardHint: '打造新武器 ▸',
    clearLoadout: '清空装备',
    startRun: '开始行动 ▸',
    continueRun: '继续行动 ▸',
    savedRunLine: '已保存的行动：第 {floor} 层，已进行 {m}:{ss} ——「继续行动」接着玩，「开始行动」会丢弃它',
  },
  store: {
    title: '商店',
    back: '锻造场',
    loading: '正在载入商店…',
    pickOne: '点击条目即可购买。价格由服务器给出。',
    empty: '当前没有可购买的商品。',
    row: '{item}   {price}',
    rowOwned: '{item}   已拥有',
    pagePrev: '‹ 上一页',
    pageNext: '下一页 ›',
    pageLabel: '第 {current}/{total} 页',
    purchasing: '正在购买 {item} —— 等待支付结果…',
    purchased: '{item} 已到账，现在就在锻造场里。',
    purchasedNoRefresh: '{item} 已到账，下次登录时会出现在锻造场里。',
    alreadyOwned: '你已经拥有 {item}。',
    busy: '上一个请求还没结束，请稍候。',
    loginRequired: '请先登录。购买绑定的是账号，不是这台设备。',
    unavailableHere: '此版本不提供购买功能。',
    notConfigured: '暂时无法购买：本版本尚未接入支付渠道。没有产生任何扣款。',
    orderFailed: '下单失败，没有产生任何扣款。',
    paymentFailed: '支付未完成，没有产生任何扣款。',
    pending: '仍在等待支付结果。若支付成功，下次登录时物品会出现。',
    listFailed: '商店载入失败，请检查网络后重试。',
  },
  hud: {
    pickupPrompt: { title: '附近武器' },
    shop: {
      title: '商店  {coins}',
      price: '{price}',
      sold: '已售出',
      buffPick: '三选一增益',
      heal: '治疗药水',
      energy: '能量电池',
      shield: '护盾电池',
      emp: '电磁脉冲手雷',
    },
    chest: {
      bigTitle: '大宝箱',
      plates: '踏板 {on}/{total}',
    },
    chips: {
      floor: '楼层',
      room: '房间',
      enemies: '敌人',
      banked: '已存',
      coins: '金币',
      score: '分数',
      buffs: '增益',
      alive: '存活',
      stage: '区域',
    },
    weapon: {
      none: '无武器',
      unarmed: '徒手',
      sub: '{rarity}·{kind}·{element}',
      damage: '伤害 {damage}',
    },
    rarity: {
      common: '普通',
      fine: '精良',
      epic: '史诗',
      legend: '传说',
      legendary: '传奇',
    },
    kind: {
      ranged: '远程',
      melee: '近战',
    },
    element: {
      physical: '物理',
      fire: '火',
      ice: '冰',
      lightning: '雷',
      poison: '毒',
    },
    elementShort: {
      physical: '物',
      fire: '火',
      ice: '冰',
      lightning: '雷',
      poison: '毒',
    },
    source: {
      purchase: '购买',
      event: '活动',
    },
    ally: {
      tag: '队友·{skin}',
      downed: '倒地 {seconds}秒',
      reviving: '救援中 {pct}%',
    },
    downed: {
      title: '你已倒地',
      bleedout: '流血倒计时——剩余 {seconds} 秒',
      reviving: '正在被救援…',
    },
    roster: { you: '你' },
    portalTitle: '楼层已清空 — 传送门已开启',
    portalExtract: '存入并撤离（{pending} 份材料）',
    portalDescend: '下降至第 {floor} 层',
    portalTitleBoss: 'BOSS 已被击败 —— 撤离传送门已开启',
    floorCardTitle: '选择一张升级卡',
    floorCardVotes: '{n} 票',
  },
  auth: {
    title: '账户',
    login: '登录',
    register: '注册',
    changePassword: '修改密码',
    logout: '登出',
    back: '返回',
    usernamePlaceholder: '用户名',
    usernameRequired: '请输入用户名。',
    passwordPlaceholder: '密码',
    currentPasswordPlaceholder: '当前密码',
    newPasswordPlaceholder: '新密码',
    loginFailed: '登录失败，请重试。',
    registerFailed: '注册失败，请重试。',
    throttled: '尝试次数过多，请几分钟后再试。',
    passwordChanged: '密码已修改。',
    passwordChangeFailed: '无法修改密码。',
    loggedInAs: '已登录：{username}',
    playingAsGuest: '以访客身份游玩',
    dataNotice:
      '可选。注册会在我们的服务器上保存你的用户名、密码哈希和游戏进度。不注册也能玩，不会上传任何数据。',
    portalDataNotice: '当你在 CrazyGames 上处于登录状态时，我们会在服务器上保存你的 CrazyGames 名称和游戏进度。',
    privacyLink: '隐私政策',
    mergeTitle: '要把访客进度带过来吗？',
    mergeBody:
      '本设备上有 {username} 没有的访客进度：{materials} 个材料、{blueprints} 张蓝图、{characters} 个角色。每台设备只会询问一次。',
    mergeUseAccount: '用账号的',
    mergeCombine: '两边合并',
    sessionExpiredTitle: '已退出登录',
    sessionExpiredBody: '保存的登录状态已失效，你现在以访客身份游玩。本设备上的进度未受影响——重新登录即可同步。',
    noticeDismiss: '好',
  },
  party: {
    title: '组队',
    create: '创建 PvP 小队',
    createCoop: '创建合作队伍',
    join: '输入邀请码加入',
    startMatching: '开始匹配',
    leave: '离开队伍',
    back: '返回',
    partyClosed: '队伍已解散。',
    createFailed: '创建队伍失败，请重试。',
    createThrottled: '创建队伍过于频繁，请几分钟后再试。',
    codePlaceholder: '邀请码',
    invalidCode: '邀请码无效或队伍已满。',
    joinThrottled: '尝试次数过多，请几分钟后再试。',
    startFailed: '开始匹配失败，请重试。',
    codeLine: '邀请码：{code}',
    you: '你',
    headerCoop: '合作 · {count}/{capacity}',
    headerSquad: 'PvP 小队 · {count}/{capacity}',
  },
  pvpPreview: {
    title: 'PVP 对战',
    map: '地图：{name}（{rooms} 个房间）',
    fairnessNote: '数值已按 PvP ×{factor} 缩放——永久装备不会带入对战。',
    queue: '开始匹配',
    back: '返回',
  },
  matchmaking: {
    searching: '正在匹配对局…',
    connecting: '正在连接…',
    elapsed: '已等待 {seconds} 秒',
    botSoon: '{seconds} 秒后由 AI 补满空位',
    botNow: '正在由 AI 补满空位…',
    partyMembers: '队伍 {count} 人',
    cancel: '取消',
    errorTitle: '匹配失败',
    retry: '重试',
    back: '返回',
    errorTimeout: '匹配等待超时。',
    errorCancelled: '匹配已取消。',
    errorGeneric: '连接失败，请重试。',
    errorThrottled: '尝试次数过多，请等几分钟后再开始匹配。',
  },
  tutorial: {
    hintMoveKeys: '用 WASD 移动。按住鼠标左键攻击——自动瞄准。',
    hintMoveTouch: '拖动屏幕左侧移动。按住右侧攻击——自动瞄准。',
    hintSwapKeys: '按 1 或 2 切换武器。',
    hintSwapTouch: '点击武器按钮切换武器。',
    hintDeflect: '用近战武器挥砍来袭子弹以将其弹反。',
    hintCleared: '完成！前往传送门。',
    skip: '跳过教程',
  },
  results: {
    mainMenuButton: '主菜单',
    extractedTitle: '撤离成功',
    defeatTitle: '战败',
    victoryTitle: '大吉大利',
    eliminatedTitle: '已淘汰',
    confirmButton: '确定',
    timeLine: '用时 {m}:{ss}',
    floorLine: '楼层 {floor}/{floorCount}',
    materialsBanked: '已存入材料：{count}',
    doubleMaterialsButton: '看广告：材料 x2',
    materialsDoubled: '已存入材料：{count}（广告奖励 x2）',
    adNotFilled: '暂无广告 —— 你的 {count} 个材料已安全存入',
    scoreLine: '分数 {score}',
    blueprintLine: '获得图纸：{weapon}',
    characterLine: '解锁角色：{character}',
    fellOnFloor: '倒在第 {floor}/{floorCount} 层',
    materialsLost: '携带的 {count} 个材料已全部丢失',
    placeOf: '第一名，共 {total} 名',
    placedOfTotal: '第 {place}/{total} 名',
    guestNotRanked: '访客不计天梯。登录后开始记分。',
    connectionLostTitle: '连接已断开',
    connectionLostBody: '无法重新连接到对局。',
  },
  toast: {
    heal: '+1 生命',
    shield: '护盾已充能',
    emp: '电磁脉冲爆发',
    newWeapon: '新武器',
    schematicFound: '图纸：{weapon}',
    characterFound: '新角色：{character}',
    buffNamed: '增益：{id}',
    buffGeneric: '增益',
    materialQty: '+{qty} {material}',
    materialFallback: '材料',
    reconnecting: '正在重新连接…',
    reconnected: '已重新连接',
    replaySaved: '录像已保存：{name}',
    replayNoRun: '当前没有可保存的对局',
    replayUnsupported: '该设备无法保存文件',
    runSaved: '行动已保存 —— 可在锻造场继续',
    runSaveFailed: '无法保存本次行动',
    runSaveOldVersion: '该存档来自更早的游戏版本',
    runSaveStale: '该存档与当前地牢已不匹配',
  },
  compareCard: {
    damage: '伤害',
    type: '类型',
    fireRate: '射速',
    spread: '散射',
    speed: '速度',
    swing: '挥砍',
    arc: '范围角度',
    reach: '距离',
    deflect: '格挡',
    yes: '是',
    no: '否',
    vs: '对比',
  },
  weapon: {
    blaster: { name: '爆能枪' },
    saber: { name: '军刀' },
    repeater: { name: '连发枪' },
    cannon: { name: '加农炮' },
    enemygun: { name: '掠夺者步枪' },
    enemyclaw: { name: '掠夺者利爪' },
    enemymaul: { name: '掠夺者重锤' },
    enemynova: { name: '掠夺者新星' },
    flamer: { name: '火焰喷射器' },
    cryobolt: { name: '寒冰箭' },
    teslagun: { name: '特斯拉枪' },
    venomspit: { name: '毒液喷射器' },
    emberblade: { name: '余烬之刃' },
    frostbrand: { name: '霜咬' },
    stormglaive: { name: '风暴长刃' },
    scattergun: { name: '霰弹枪' },
    seeker: { name: '追踪者' },
    mortar: { name: '迫击炮' },
    lasercutter: { name: '激光切割器' },
    tomahawk: { name: '战斧' },
    hammer: { name: '战锤' },
    spear: { name: '长枪' },
    novaburst: { name: '新星爆裂' },
    gyre: { name: '旋涡' },
    carom: { name: '弹跳弹' },
    leech: { name: '嗜血刃' },
    cinderscatter: { name: '炽炭散射' },
    frostseeker: { name: '冰霜追猎者' },
  },
  skin: {
    vanguard: { name: '先锋' },
    skirmisher: { name: '游击手' },
    juggernaut: { name: '重装兵' },
  },
  card: {
    potion_flow: { name: '流泉', desc: '血瓶掉落率 {factor} 倍' },
    windfall: { name: '横财', desc: '金币价值 {factor} 倍' },
    edge: { name: '锋锐', desc: '伤害 +{value}%' },
    cadence: { name: '连射', desc: '攻击速度 +{value}%' },
    bulwark: { name: '壁垒', desc: '生命上限 +{value}' },
    precision: { name: '精准', desc: '暴击率 +{value}%' },
    capacitor: { name: '电容', desc: '能量上限 +{value}' },
    surge: { name: '涌流', desc: '能量价值 {factor} 倍' },
    aegis: { name: '神盾', desc: '护盾价值 {factor} 倍' },
    stockpile: { name: '囤积', desc: '材料价值 {factor} 倍' },
    bounty: { name: '赏金', desc: '每个宝箱 +{count} 把武器' },
  },
  buff: {
    dmg_up: { name: '伤害提升' },
    rof_up: { name: '射速提升' },
    vit_up: { name: '体力提升' },
    crit_up: { name: '暴击率提升' },
    cell_up: { name: '能量上限提升' },
  },
  material: {
    physical: { name: '物理' },
    fire: { name: '火' },
    ice: { name: '冰' },
    lightning: { name: '雷' },
    poison: { name: '毒' },
  },
};
