/**
 * Russian translation (design/17-i18n.md). Must mirror `en.ts`'s exact key shape —
 * `Translations<typeof en>` (../index.ts) makes a missing/extra key a compile error.
 *
 * Russian nouns decline differently by count (1 / 2-4 / 5+) — same problem
 * `results.materialsBanked` hit in English at count=1. Rather than build a plural
 * engine for one language, count-adjacent nouns here use the same "label: number"
 * phrasing en.ts's fix introduced, which reads naturally regardless of count.
 */
import type { Translations } from '..';
import type { en } from './en';

export const ru: Translations<typeof en> = {
  mainMenu: {
    title: 'BLIGHTBLOOM',
    subtitle: 'спускайся, добывай, выживай',
    play: 'ИГРАТЬ',
    continueRun: 'ПРОДОЛЖИТЬ ▸',
    continueRunAt: 'Этаж {floor} · {m}:{ss} игры',
    solo: 'СОЛО PvE',
    coop: 'КООП',
    pvpSolo: 'СОЛО-ОЧЕРЕДЬ PVP',
    tutorial: 'ОБУЧЕНИЕ',
    recommended: 'ВПЕРВЫЕ ЗДЕСЬ?',
    squad: 'ОТРЯД',
    forge: 'КУЗНИЦА',
    account: 'ВХОД',
    settings: 'НАСТРОЙКИ',
    greeting: 'Привет, {username}',
  },
  loading: {
    boot: 'ЗАГРУЗКА',
    art: 'ЗАГРУЗКА ГРАФИКИ',
    enteringRun: 'ВХОД В ПОДЗЕМЕЛЬЕ',
    returningToHub: 'ВОЗВРАЩЕНИЕ В ЛОББИ',
  },
  pauseMenu: {
    title: 'ПАУЗА',
    resume: 'ПРОДОЛЖИТЬ',
    settings: 'НАСТРОЙКИ',
    saveQuit: 'СОХРАНИТЬ',
    quit: 'В КУЗНИЦУ',
  },
  settings: {
    title: 'НАСТРОЙКИ',
    master: 'Общая',
    sfx: 'Эффекты',
    music: 'Музыка',
    mute: 'БЕЗ ЗВУКА',
    unmute: 'ВКЛЮЧИТЬ ЗВУК',
    language: 'ЯЗЫК: {name}',
    controlLayout: 'УПРАВЛЕНИЕ: {mode}',
    controlLayoutStandard: 'СТАНДАРТ',
    controlLayoutMirrored: 'ЛЕВША',
    quality: 'КАЧЕСТВО: {mode}',
    qualityAuto: 'АВТО',
    qualityHigh: 'ВЫСОКОЕ',
    qualityLow: 'НИЗКОЕ',
    qualityAutoLow: 'АВТО (НИЗКОЕ)',
    qualityMedium: 'СРЕДНЕЕ',
    qualityAutoMedium: 'АВТО (СРЕДНЕЕ)',
    frameRate: 'ЧАСТОТА КАДРОВ: {fps}',
    reduceMotion: 'МЕНЬШЕ ТРЯСКИ: {mode}',
    on: 'ВКЛ.',
    off: 'ВЫКЛ.',
    tutorial: 'ПОВТОРИТЬ ОБУЧЕНИЕ',
    back: 'НАЗАД',
  },
  forge: {
    title: 'ФОРПОСТ КУЗНИЦЫ',
    hint: '[↑↓] обзор · [1-9] создать · [B] магазин',
    backButton: '← НАЗАД',
    pagePrevButton: '‹ СТРАНИЦА',
    pageNextButton: 'СТРАНИЦА ›',
    noneStarterPair: '(нет → {weapons})',
    moreAvailable: 'ещё доступно: {count}',
    materialsLine: 'Материалы   {bank}',
    loadoutLine: 'Снаряжение  {loadout}   ({count}/{max})',
    storeLine: 'Магазин: {items}  — [B] открыть магазин',
    storeButton: 'МАГАЗИН',
    lockedFind: 'заблокировано (нужно найти)',
    lockedSource: 'заблокировано ({source})',
    craftable: 'можно скрафтить',
    craftableSchematic: 'можно скрафтить (схема ×{count})',
    needMaterials: 'не хватает материалов',
    kindTaken: 'тип уже занят',
    stagedTag: '  ▸экипировано×{count}',
    pageLabel: 'Страница {current}/{total}',
    equippedHeader: 'Экипировано: {id}',
    candidateHeader: 'Кандидат: {id}',
  },
  loadout: {
    title: 'СНАРЯЖЕНИЕ',
    hint: '[C] персонаж · [X] очистить · [F] кузница · [Enter] старт',
    backButton: '← МЕНЮ',
    charStats: '{hp} ОЗ / {sh} Щит',
    ownedChars: 'персонажей в наличии: {count}',
    materialsLine: 'Материалы   {bank}',
    slotsLine: 'Оружие с собой   {count}/{max}',
    craftedTag: 'выковано',
    starterTag: 'базовый набор',
    forgeCardName: 'КУЗНИЦА',
    forgeCardHint: 'ковать оружие ▸',
    clearLoadout: 'ОЧИСТИТЬ',
    startRun: 'НАЧАТЬ ЗАБЕГ ▸',
    continueRun: 'ПРОДОЛЖИТЬ ▸',
    savedRunLine: 'Сохранённый забег: этаж {floor}, {m}:{ss} игры — ПРОДОЛЖИТЬ вернёт вас в него, НАЧАТЬ ЗАБЕГ его удалит',
  },
  store: {
    title: 'МАГАЗИН',
    back: 'КУЗНИЦА',
    loading: 'Загрузка магазина…',
    pickOne: 'Нажмите на товар, чтобы купить его. Цены задаёт сервер.',
    empty: 'Сейчас ничего не продаётся.',
    row: '{item}   {price}',
    rowOwned: '{item}   В НАЛИЧИИ',
    pagePrev: '‹ СТРАНИЦА',
    pageNext: 'СТРАНИЦА ›',
    pageLabel: 'Страница {current}/{total}',
    purchasing: 'Покупаем {item} — ждём подтверждения оплаты…',
    purchased: '{item} — ваше. Уже в кузнице.',
    purchasedNoRefresh: '{item} — ваше. Появится в кузнице при следующем входе.',
    alreadyOwned: 'У вас уже есть {item}.',
    busy: 'По одному запросу за раз — предыдущий ещё выполняется.',
    loginRequired: 'Сначала войдите в аккаунт. Покупки привязаны к аккаунту, а не к устройству.',
    unavailableHere: 'В этой версии игры покупки недоступны.',
    notConfigured: 'Покупки сейчас недоступны: в этой сборке не настроен платёжный провайдер. Ничего не списано.',
    orderFailed: 'Не удалось начать покупку. Ничего не списано.',
    paymentFailed: 'Оплата не прошла. Ничего не списано.',
    pending: 'Оплата ещё обрабатывается. Если она пройдёт, предмет появится при следующем входе.',
    listFailed: 'Не удалось загрузить магазин. Проверьте соединение и попробуйте снова.',
  },
  hud: {
    pickupPrompt: { title: 'Оружие рядом' },
    shop: {
      title: 'Лавка  {coins}',
      price: '{price}',
      sold: 'ПРОДАНО',
      buffPick: 'Выбери усиление',
      heal: 'Зелье лечения',
      energy: 'Энергоячейка',
      shield: 'Батарея щита',
      emp: 'Граната ЭМИ',
    },
    chest: {
      bigTitle: 'Большой сундук',
      plates: 'Плиты заняты {on}/{total}',
    },
    chips: {
      floor: 'ЭТАЖ',
      room: 'КОМНАТА',
      enemies: 'ВРАГИ',
      banked: 'СОХРАНЕНО',
      coins: 'МОНЕТЫ',
      score: 'ОЧКИ',
      buffs: 'БАФФЫ',
      alive: 'ЖИВЫ',
      stage: 'ЗОНА',
    },
    weapon: {
      none: 'НЕТ ОРУЖИЯ',
      unarmed: 'безоружен',
      sub: '{rarity} · {kind} · {element}',
      damage: 'УРОН {damage}',
    },
    rarity: {
      common: 'обычное',
      fine: 'доброе',
      epic: 'эпическое',
      legend: 'легендарное',
      legendary: 'древнее',
    },
    kind: {
      ranged: 'дальнее',
      melee: 'ближнее',
    },
    element: {
      physical: 'физический',
      fire: 'огонь',
      ice: 'лёд',
      lightning: 'молния',
      poison: 'яд',
    },
    elementShort: {
      physical: 'ФИЗ',
      fire: 'ОГН',
      ice: 'ЛЁД',
      lightning: 'МОЛ',
      poison: 'ЯД',
    },
    source: {
      purchase: 'покупка',
      event: 'событие',
    },
    ally: {
      tag: 'СОЮЗНИК · {skin}',
      downed: 'СБИТ {seconds}с',
      reviving: 'РЕАНИМАЦИЯ {pct}%',
    },
    downed: {
      title: 'ВЫ СБИТЫ',
      bleedout: 'Кровотечение — осталось {seconds}с',
      reviving: 'ВАС РЕАНИМИРУЮТ…',
    },
    roster: { you: 'ВЫ' },
    portalTitle: 'ЭТАЖ ЗАЧИЩЕН — портал открылся',
    portalExtract: 'Сохранить и эвакуироваться ({pending} материалов)',
    portalDescend: 'Спуститься на этаж {floor}',
    portalTitleBoss: 'БОСС ПОВЕРЖЕН — портал эвакуации открыт',
    floorCardTitle: 'Выберите улучшение',
    floorCardVotes: '{n} голосов',
  },
  auth: {
    title: 'АККАУНТ',
    login: 'ВОЙТИ',
    register: 'РЕГИСТРАЦИЯ',
    changePassword: 'СМЕНИТЬ ПАРОЛЬ',
    logout: 'ВЫЙТИ',
    back: 'НАЗАД',
    usernamePlaceholder: 'ИМЯ ПОЛЬЗОВАТЕЛЯ',
    usernameRequired: 'Требуется имя пользователя.',
    passwordPlaceholder: 'ПАРОЛЬ',
    currentPasswordPlaceholder: 'ТЕКУЩИЙ ПАРОЛЬ',
    newPasswordPlaceholder: 'НОВЫЙ ПАРОЛЬ',
    loginFailed: 'Не удалось войти — попробуйте снова.',
    registerFailed: 'Не удалось зарегистрироваться — попробуйте снова.',
    throttled: 'Слишком много попыток — повторите через несколько минут.',
    passwordChanged: 'Пароль изменён.',
    passwordChangeFailed: 'Не удалось изменить пароль.',
    loggedInAs: 'Вход выполнен как {username}',
    playingAsGuest: 'Игра как гость',
    dataNotice:
      'Необязательно. При регистрации мы храним имя пользователя, хеш пароля и прогресс на нашем сервере. Без аккаунта ничего не отправляется.',
    portalDataNotice: 'Пока вы вошли в аккаунт CrazyGames, мы храним ваше имя в CrazyGames и игровой прогресс на нашем сервере.',
    privacyLink: 'Политика конфиденциальности',
    mergeTitle: 'ПЕРЕНЕСТИ ГОСТЕВОЙ ПРОГРЕСС?',
    mergeBody:
      'На этом устройстве есть гостевой прогресс, которого нет у {username}: материалов — {materials}, чертежей — {blueprints}, персонажей — {characters}. Вопрос задаётся один раз на устройство.',
    mergeUseAccount: 'ВЗЯТЬ ИЗ АККАУНТА',
    mergeCombine: 'ОБЪЕДИНИТЬ',
    sessionExpiredTitle: 'ВЫПОЛНЕН ВЫХОД',
    sessionExpiredBody:
      'Сохранённая сессия больше недействительна, поэтому вы играете как гость. Прогресс на этом устройстве не тронут — войдите снова, чтобы синхронизировать его.',
    noticeDismiss: 'OK',
  },
  party: {
    title: 'ОТРЯД',
    create: 'СОЗДАТЬ ОТРЯД',
    createCoop: 'СОЗДАТЬ КООП-ГРУППУ',
    join: 'ВСТУПИТЬ ПО КОДУ',
    startMatching: 'НАЧАТЬ ПОИСК',
    leave: 'ПОКИНУТЬ ОТРЯД',
    back: 'НАЗАД',
    partyClosed: 'Отряд распущен.',
    createFailed: 'Не удалось создать отряд — попробуйте снова.',
    createThrottled: 'Создано слишком много отрядов — повторите через несколько минут.',
    codePlaceholder: 'КОД',
    invalidCode: 'Неверный код или отряд заполнен.',
    joinThrottled: 'Слишком много попыток — повторите через несколько минут.',
    startFailed: 'Не удалось начать поиск — попробуйте снова.',
    codeLine: 'КОД: {code}',
    you: 'ты',
    headerCoop: 'КООП · {count}/{capacity}',
    headerSquad: 'PVP-ОТРЯД · {count}/{capacity}',
  },
  pvpPreview: {
    title: 'PVP-МАТЧ',
    map: 'Карта: {name} ({rooms} комнат)',
    fairnessNote: 'Характеристики масштабированы ×{factor} для PvP — постоянное снаряжение никогда не переносится.',
    queue: 'ИСКАТЬ',
    back: 'НАЗАД',
  },
  matchmaking: {
    searching: 'Поиск матча…',
    connecting: 'Подключение…',
    elapsed: 'прошло {seconds}с',
    botSoon: 'ИИ-игроки займут свободные места через {seconds}с',
    botNow: 'Свободные места занимают ИИ-игроки…',
    partyMembers: 'в отряде: {count}',
    cancel: 'ОТМЕНА',
    errorTitle: 'МАТЧ НЕ СОСТОЯЛСЯ',
    retry: 'ПОВТОРИТЬ',
    back: 'НАЗАД',
    errorTimeout: 'Истекло время ожидания матча.',
    errorCancelled: 'Поиск отменён.',
    errorGeneric: 'Не удалось подключиться — попробуйте снова.',
    errorThrottled: 'Слишком много попыток — подождите несколько минут перед новым поиском.',
  },
  tutorial: {
    hintMoveKeys: 'Движение — WASD. Удерживайте левую кнопку мыши для атаки: прицеливание автоматическое.',
    hintMoveTouch: 'Ведите пальцем по левой части экрана, чтобы двигаться, и удерживайте правую для атаки: прицеливание автоматическое.',
    hintSwapKeys: 'Нажмите 1 или 2, чтобы сменить оружие.',
    hintSwapTouch: 'Коснитесь кнопки оружия, чтобы сменить оружие.',
    hintDeflect: 'Взмахните оружием ближнего боя навстречу пулям, чтобы отразить их.',
    hintCleared: 'Отлично — теперь к порталу.',
    skip: 'ПРОПУСТИТЬ ОБУЧЕНИЕ',
  },
  results: {
    mainMenuButton: 'ГЛАВНОЕ МЕНЮ',
    extractedTitle: 'ЭВАКУАЦИЯ УСПЕШНА',
    defeatTitle: 'ПОРАЖЕНИЕ',
    victoryTitle: 'ПОБЕДА РОЯЛЬ',
    eliminatedTitle: 'ВЫБЫЛ',
    confirmButton: 'ПОДТВЕРДИТЬ',
    timeLine: 'Время {m}:{ss}',
    floorLine: 'Этаж {floor}/{floorCount}',
    materialsBanked: 'Сохранено материалов: {count}',
    doubleMaterialsButton: 'СМОТРЕТЬ РЕКЛАМУ: МАТЕРИАЛЫ x2',
    materialsDoubled: 'Сохранено материалов: {count} (бонус за рекламу x2)',
    adNotFilled: 'Реклама недоступна — ваши материалы сохранены: {count}',
    scoreLine: 'Очки {score}',
    blueprintLine: 'Чертёж получен: {weapon}',
    characterLine: 'Персонаж открыт: {character}',
    fellOnFloor: 'Пал на этаже {floor}/{floorCount}',
    materialsLost: 'Все переносимые материалы потеряны: {count}',
    placeOf: '1-е место из {total}',
    placedOfTotal: 'Место {place}/{total}',
    guestNotRanked: 'Гость: без рейтинга. Войдите, чтобы попасть.',
    connectionLostTitle: 'СОЕДИНЕНИЕ ПОТЕРЯНО',
    connectionLostBody: 'Не удалось переподключиться к матчу.',
  },
  toast: {
    heal: '+1 ОЗ',
    shield: 'Щит перезаряжен',
    emp: 'Взрыв ЭМИ',
    newWeapon: 'Новое оружие',
    schematicFound: 'Схема: {weapon}',
    characterFound: 'Новый персонаж: {character}',
    buffNamed: 'Бафф: {id}',
    buffGeneric: 'Бафф',
    materialQty: '+{qty} {material}',
    materialFallback: 'материал',
    reconnecting: 'Переподключение…',
    reconnected: 'Переподключено',
    replaySaved: 'Запись сохранена: {name}',
    replayNoRun: 'Пока нечего сохранять',
    replayUnsupported: 'Это устройство не может сохранять файлы',
    runSaved: 'Забег сохранён — продолжите его в кузнице',
    runSaveFailed: 'Не удалось сохранить этот забег',
    runSaveOldVersion: 'Этот сохранённый забег из более старой версии игры',
    runSaveStale: 'Этот сохранённый забег больше не соответствует этому подземелью',
  },
  compareCard: {
    damage: 'Урон',
    type: 'Тип',
    fireRate: 'Скорострельность',
    spread: 'Разброс',
    speed: 'Скорость',
    swing: 'Замах',
    arc: 'Дуга',
    reach: 'Дальность',
    deflect: 'Отражение',
    yes: 'да',
    no: 'нет',
    vs: 'vs',
  },
  weapon: {
    blaster: { name: 'Бластер' },
    saber: { name: 'Сабля' },
    repeater: { name: 'Репитер' },
    cannon: { name: 'Пушка' },
    enemygun: { name: 'Винтовка мародёра' },
    enemyclaw: { name: 'Коготь мародёра' },
    enemymaul: { name: 'Молот мародёра' },
    enemynova: { name: 'Нова мародёра' },
    flamer: { name: 'Огнемёт' },
    cryobolt: { name: 'Криоболт' },
    teslagun: { name: 'Пушка Теслы' },
    venomspit: { name: 'Ядомёт' },
    emberblade: { name: 'Клинок углей' },
    frostbrand: { name: 'Морозный клинок' },
    stormglaive: { name: 'Глефа бури' },
    scattergun: { name: 'Дробовик' },
    seeker: { name: 'Искатель' },
    mortar: { name: 'Миномёт' },
    lasercutter: { name: 'Лазерный резак' },
    tomahawk: { name: 'Томагавк' },
    hammer: { name: 'Молот' },
    spear: { name: 'Копьё' },
    novaburst: { name: 'Вспышка новы' },
    gyre: { name: 'Вихрь' },
    carom: { name: 'Рикошет' },
    leech: { name: 'Пиявка' },
    cinderscatter: { name: 'Россыпь углей' },
    frostseeker: { name: 'Морозный искатель' },
  },
  skin: {
    vanguard: { name: 'Авангард' },
    skirmisher: { name: 'Застрельщик' },
    juggernaut: { name: 'Джаггернаут' },
  },
  card: {
    potion_flow: { name: 'Живительный поток', desc: 'Зелья выпадают в {factor} раза чаще' },
    windfall: { name: 'Находка', desc: 'Монеты стоят {factor}x' },
    edge: { name: 'Острота', desc: '+{value}% урона' },
    cadence: { name: 'Темп', desc: '+{value}% скорости стрельбы' },
    bulwark: { name: 'Бастион', desc: '+{value} к макс. ЗД' },
    precision: { name: 'Точность', desc: '+{value}% шанса крита' },
    capacitor: { name: 'Конденсатор', desc: '+{value} к макс. энергии' },
    surge: { name: 'Импульс', desc: 'Энергия стоит {factor}x' },
    aegis: { name: 'Эгида', desc: 'Щиты стоят {factor}x' },
    stockpile: { name: 'Запас', desc: 'Материалы стоят {factor}x' },
    bounty: { name: 'Добыча', desc: '+{count} оружия за сундук' },
  },
  buff: {
    dmg_up: { name: 'Урон увеличен' },
    rof_up: { name: 'Скорострельность увеличена' },
    vit_up: { name: 'Живучесть увеличена' },
    crit_up: { name: 'Шанс крита увеличен' },
    cell_up: { name: 'Макс. энергия увеличена' },
  },
  material: {
    physical: { name: 'Физический' },
    fire: { name: 'Огонь' },
    ice: { name: 'Лёд' },
    lightning: { name: 'Молния' },
    poison: { name: 'Яд' },
  },
};
