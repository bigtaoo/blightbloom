/**
 * French translation (design/17-i18n.md). Must mirror `en.ts`'s exact key shape —
 * `Translations<typeof en>` (../index.ts) makes a missing/extra key a compile error.
 */
import type { Translations } from '..';
import type { en } from './en';

export const fr: Translations<typeof en> = {
  mainMenu: {
    title: 'BLIGHTBLOOM',
    subtitle: 'descendre, extraire, survivre',
    play: 'JOUER',
    continueRun: 'CONTINUER ▸',
    continueRunAt: 'Étage {floor} · {m}:{ss} joué',
    solo: 'SOLO PvE',
    coop: 'COOP',
    pvpSolo: 'FILE PVP SOLO',
    tutorial: 'TUTORIEL',
    recommended: 'NOUVEAU ICI ?',
    squad: 'ESCOUADE',
    forge: 'FORGE',
    account: 'CONNEXION',
    settings: 'PARAMÈTRES',
    greeting: 'Salut, {username}',
  },
  loading: {
    boot: 'CHARGEMENT',
    art: 'CHARGEMENT DES GRAPHISMES',
    enteringRun: 'ENTRÉE DANS LE DONJON',
    returningToHub: 'RETOUR AU HALL',
  },
  pauseMenu: {
    title: 'EN PAUSE',
    resume: 'REPRENDRE',
    settings: 'PARAMÈTRES',
    saveQuit: 'SAUVER & QUITTER',
    quit: 'RETOUR À LA FORGE',
  },
  settings: {
    title: 'PARAMÈTRES',
    master: 'Général',
    sfx: 'Effets',
    music: 'Musique',
    mute: 'MUET',
    unmute: 'RÉACTIVER',
    language: 'LANGUE : {name}',
    controlLayout: 'COMMANDES : {mode}',
    controlLayoutStandard: 'STANDARD',
    controlLayoutMirrored: 'GAUCHER',
    quality: 'QUALITÉ : {mode}',
    qualityAuto: 'AUTO',
    qualityHigh: 'ÉLEVÉE',
    qualityLow: 'BASSE',
    qualityAutoLow: 'AUTO (BASSE)',
    qualityMedium: 'MOYENNE',
    qualityAutoMedium: 'AUTO (MOYENNE)',
    frameRate: 'FRÉQUENCE : {fps}',
    reduceMotion: 'RÉDUIRE LES MOUVEMENTS : {mode}',
    on: 'OUI',
    off: 'NON',
    tutorial: 'REVOIR LE TUTORIEL',
    back: 'RETOUR',
  },
  forge: {
    title: 'AVANT-POSTE DE LA FORGE',
    hint: '[↑↓] parcourir · [1-9] fabriquer · [B] boutique',
    backButton: '← RETOUR',
    pagePrevButton: '‹ PAGE',
    pageNextButton: 'PAGE ›',
    noneStarterPair: '(aucune → {weapons})',
    moreAvailable: '{count} de plus disponibles',
    materialsLine: 'Matériaux   {bank}',
    loadoutLine: 'Équipement  {loadout}   ({count}/{max})',
    storeLine: 'Boutique : {items}  — [B] ouvrir la boutique',
    storeButton: 'BOUTIQUE',
    lockedFind: 'verrouillé (à trouver)',
    lockedSource: 'verrouillé ({source})',
    craftable: 'fabricable',
    craftableSchematic: 'fabricable (schéma ×{count})',
    needMaterials: 'matériaux manquants',
    kindTaken: 'type déjà pris',
    stagedTag: '  ▸équipé×{count}',
    pageLabel: 'Page {current}/{total}',
    equippedHeader: 'Équipé : {id}',
    candidateHeader: 'Candidat : {id}',
  },
  loadout: {
    title: 'ÉQUIPEMENT',
    hint: '[C] personnage · [X] vider · [F] forge · [Entrée] démarrer',
    backButton: '← MENU',
    charStats: '{hp} PV / {sh} Bouclier',
    ownedChars: 'personnages possédés : {count}',
    materialsLine: 'Matériaux   {bank}',
    slotsLine: 'Armes emportées   {count}/{max}',
    craftedTag: 'forgée',
    starterTag: 'kit par défaut',
    forgeCardName: 'FORGE',
    forgeCardHint: 'fabriquer des armes ▸',
    clearLoadout: 'VIDER',
    startRun: 'DÉMARRER ▸',
    continueRun: 'CONTINUER ▸',
    savedRunLine: "Partie sauvegardée : étage {floor}, {m}:{ss} joué — CONTINUER pour reprendre, DÉMARRER l'abandonne",
  },
  store: {
    title: 'BOUTIQUE',
    back: 'FORGE',
    loading: 'Chargement de la boutique…',
    pickOne: 'Touchez un article pour l’acheter. Les prix viennent du serveur.',
    empty: 'Rien n’est en vente pour le moment.',
    row: '{item}   {price}',
    rowOwned: '{item}   POSSÉDÉ',
    pagePrev: '‹ PAGE',
    pageNext: 'PAGE ›',
    pageLabel: 'Page {current}/{total}',
    purchasing: 'Achat de {item} — en attente du paiement…',
    purchased: '{item} est à vous. C’est déjà dans la forge.',
    purchasedNoRefresh: '{item} est à vous. L’objet apparaîtra dans la forge à votre prochaine connexion.',
    alreadyOwned: 'Vous possédez déjà {item}.',
    busy: 'Une chose à la fois — la requête précédente est encore en cours.',
    loginRequired: 'Connectez-vous d’abord. Les achats sont liés à votre compte, pas à cet appareil.',
    unavailableHere: 'Les achats ne sont pas disponibles dans cette version du jeu.',
    notConfigured: 'Achats indisponibles pour le moment : aucun prestataire de paiement n’est configuré dans cette version. Rien n’a été débité.',
    orderFailed: 'Impossible de lancer l’achat. Rien n’a été débité.',
    paymentFailed: 'Le paiement n’a pas abouti. Rien n’a été débité.',
    pending: 'Paiement toujours en attente. S’il aboutit, l’objet apparaîtra à votre prochaine connexion.',
    listFailed: 'Impossible de charger la boutique. Vérifiez votre connexion et réessayez.',
  },
  hud: {
    pickupPrompt: { title: 'Armes à proximité' },
    shop: {
      title: 'Boutique  {coins}',
      price: '{price}',
      sold: 'VENDU',
      buffPick: 'Choisis un bonus',
      heal: 'Potion de soin',
      energy: "Cellule d'énergie",
      shield: 'Batterie de bouclier',
      emp: 'Grenade EMP',
    },
    chest: {
      bigTitle: 'Grand coffre',
      plates: 'Plaques occupées {on}/{total}',
    },
    chips: {
      floor: 'ÉTAGE',
      room: 'SALLE',
      enemies: 'ENNEMIS',
      banked: 'STOCKÉ',
      coins: 'PIÈCES',
      score: 'SCORE',
      buffs: 'BONUS',
      alive: 'VIVANTS',
      stage: 'ZONE',
    },
    weapon: {
      none: 'AUCUNE ARME',
      unarmed: 'à mains nues',
      sub: '{rarity} · {kind} · {element}',
      damage: 'DÉGÂTS {damage}',
    },
    rarity: {
      common: 'commune',
      fine: 'raffinée',
      epic: 'épique',
      legend: 'légendaire',
      legendary: 'ancestrale',
    },
    kind: {
      ranged: 'distance',
      melee: 'corps à corps',
    },
    element: {
      physical: 'physique',
      fire: 'feu',
      ice: 'glace',
      lightning: 'foudre',
      poison: 'poison',
    },
    elementShort: {
      physical: 'PHY',
      fire: 'FEU',
      ice: 'GLA',
      lightning: 'FOU',
      poison: 'POI',
    },
    source: {
      purchase: 'achat',
      event: 'événement',
    },
    ally: {
      tag: 'ALLIÉ · {skin}',
      downed: 'À TERRE {seconds}s',
      reviving: 'RÉANIMATION {pct}%',
    },
    downed: {
      title: 'VOUS ÊTES À TERRE',
      bleedout: 'Saignement — {seconds}s restantes',
      reviving: 'EN COURS DE RÉANIMATION…',
    },
    roster: { you: 'VOUS' },
    portalTitle: 'ÉTAGE NETTOYÉ — un portail s\'est ouvert',
    portalExtract: 'Stocker et extraire ({pending} matériaux)',
    portalDescend: "Descendre à l'étage {floor}",
    portalTitleBoss: "BOSS VAINCU — le portail d'extraction est ouvert",
    floorCardTitle: 'Choisissez une amélioration',
    floorCardVotes: '{n} votes',
  },
  auth: {
    title: 'COMPTE',
    login: 'CONNEXION',
    register: "S'INSCRIRE",
    changePassword: 'CHANGER LE MDP',
    logout: 'DÉCONNEXION',
    back: 'RETOUR',
    usernamePlaceholder: "NOM D'UTILISATEUR",
    usernameRequired: "Nom d'utilisateur requis.",
    passwordPlaceholder: 'MOT DE PASSE',
    currentPasswordPlaceholder: 'MOT DE PASSE ACTUEL',
    newPasswordPlaceholder: 'NOUVEAU MOT DE PASSE',
    loginFailed: 'Échec de la connexion — réessayez.',
    registerFailed: "Échec de l'inscription — réessayez.",
    throttled: 'Trop de tentatives — réessayez dans quelques minutes.',
    passwordChanged: 'Mot de passe changé.',
    passwordChangeFailed: 'Impossible de changer le mot de passe.',
    loggedInAs: 'Connecté en tant que {username}',
    playingAsGuest: 'Jouer en tant qu\'invité',
    dataNotice:
      'Facultatif. L’inscription enregistre votre nom d’utilisateur, une empreinte du mot de passe et votre progression sur notre serveur. Sans compte, rien n’est envoyé.',
    portalDataNotice: 'Lorsque vous êtes connecté sur CrazyGames, nous conservons votre nom CrazyGames et votre progression sur notre serveur.',
    privacyLink: 'Politique de confidentialité',
    mergeTitle: 'REPRENDRE LA PROGRESSION INVITÉ ?',
    mergeBody:
      "Cet appareil contient une progression invité que {username} n'a pas : {materials} matériaux, {blueprints} plans, {characters} personnages. La question n'est posée qu'une fois par appareil.",
    mergeUseAccount: 'UTILISER LE COMPTE',
    mergeCombine: 'COMBINER LES DEUX',
    sessionExpiredTitle: 'DÉCONNECTÉ',
    sessionExpiredBody:
      "Votre session enregistrée n'est plus valide, vous jouez donc en tant qu'invité. La progression de cet appareil est intacte — reconnectez-vous pour la synchroniser.",
    noticeDismiss: 'OK',
  },
  party: {
    title: 'ESCOUADE',
    create: 'CRÉER UNE ÉQUIPE',
    createCoop: 'CRÉER UN GROUPE COOP',
    join: 'REJOINDRE AVEC UN CODE',
    startMatching: 'LANCER LA RECHERCHE',
    leave: "QUITTER L'ESCOUADE",
    back: 'RETOUR',
    partyClosed: 'Escouade dissoute.',
    createFailed: "Impossible de créer l'escouade — réessayez.",
    createThrottled: "Trop d'escouades créées — réessayez dans quelques minutes.",
    codePlaceholder: 'CODE',
    invalidCode: 'Code invalide ou escouade complète.',
    joinThrottled: 'Trop de tentatives — réessayez dans quelques minutes.',
    startFailed: 'Impossible de lancer la recherche — réessayez.',
    codeLine: 'CODE : {code}',
    you: 'toi',
    headerCoop: 'COOP · {count}/{capacity}',
    headerSquad: 'ESCOUADE JCJ · {count}/{capacity}',
  },
  pvpPreview: {
    title: 'MATCH PVP',
    map: 'Carte : {name} ({rooms} salles)',
    fairnessNote: 'Stats mises à l’échelle ×{factor} pour le PvP — l’équipement permanent n’est jamais importé.',
    queue: 'RECHERCHER',
    back: 'RETOUR',
  },
  matchmaking: {
    searching: "Recherche d'un match…",
    connecting: 'Connexion…',
    elapsed: '{seconds}s écoulées',
    botSoon: 'Des joueurs IA rejoignent dans {seconds}s',
    botNow: 'Ajout de joueurs IA aux places libres…',
    partyMembers: '{count} dans l\'escouade',
    cancel: 'ANNULER',
    errorTitle: 'ÉCHEC DU MATCH',
    retry: 'RÉESSAYER',
    back: 'RETOUR',
    errorTimeout: "Délai d'attente dépassé pour trouver un match.",
    errorCancelled: 'Recherche annulée.',
    errorGeneric: 'Connexion impossible — réessayez.',
    errorThrottled: 'Trop de tentatives — attendez quelques minutes avant de rechercher à nouveau.',
  },
  tutorial: {
    hintMoveKeys: 'Déplacez-vous avec ZQSD. Maintenez le clic gauche pour attaquer — la visée est automatique.',
    hintMoveTouch: 'Faites glisser le côté gauche de l’écran pour vous déplacer, maintenez le côté droit pour attaquer — la visée est automatique.',
    hintSwapKeys: 'Appuyez sur 1 ou 2 pour changer d’arme.',
    hintSwapTouch: 'Touchez un bouton d’arme pour changer d’arme.',
    hintDeflect: 'Frappez les balles entrantes avec votre arme de mêlée pour les dévier.',
    hintCleared: 'Bien joué — direction le portail.',
    skip: 'PASSER LE TUTORIEL',
  },
  results: {
    mainMenuButton: 'MENU PRINCIPAL',
    extractedTitle: 'EXTRACTION RÉUSSIE',
    defeatTitle: 'DÉFAITE',
    victoryTitle: 'VICTOIRE ROYALE',
    eliminatedTitle: 'ÉLIMINÉ',
    confirmButton: 'CONFIRMER',
    timeLine: 'Temps {m}:{ss}',
    floorLine: 'Étage {floor}/{floorCount}',
    materialsBanked: 'Matériaux stockés : {count}',
    doubleMaterialsButton: 'VOIR UNE PUB : MATÉRIAUX x2',
    materialsDoubled: 'Matériaux stockés : {count} (bonus pub x2)',
    adNotFilled: 'Aucune pub disponible — vos {count} matériaux sont conservés',
    scoreLine: 'Score {score}',
    blueprintLine: 'Plan récupéré : {weapon}',
    characterLine: 'Personnage débloqué : {character}',
    fellOnFloor: 'Tombé à l\'étage {floor}/{floorCount}',
    materialsLost: 'Les {count} matériaux transportés ont été perdus',
    placeOf: '1re place sur {total}',
    placedOfTotal: 'Classé {place}/{total}',
    guestNotRanked: 'Invité : non classé. Connectez-vous pour l\'être.',
    connectionLostTitle: 'CONNEXION PERDUE',
    connectionLostBody: 'Impossible de se reconnecter à la partie.',
  },
  toast: {
    heal: '+1 PV',
    shield: 'Bouclier rechargé',
    emp: 'Explosion EMP',
    newWeapon: 'Nouvelle arme',
    schematicFound: 'Schéma : {weapon}',
    characterFound: 'Nouveau personnage : {character}',
    buffNamed: 'Bonus : {id}',
    buffGeneric: 'Bonus',
    materialQty: '+{qty} {material}',
    materialFallback: 'matériau',
    reconnecting: 'Reconnexion…',
    reconnected: 'Reconnecté',
    replaySaved: 'Replay enregistré : {name}',
    replayNoRun: 'Rien à enregistrer pour le moment',
    replayUnsupported: 'Cet appareil ne peut pas enregistrer de fichiers',
    runSaved: 'Partie sauvegardée — reprenez-la depuis la forge',
    runSaveFailed: 'Impossible de sauvegarder cette partie',
    runSaveOldVersion: "Cette sauvegarde vient d'une version antérieure du jeu",
    runSaveStale: 'Cette sauvegarde ne correspond plus à ce donjon',
  },
  compareCard: {
    damage: 'Dégâts',
    type: 'Type',
    fireRate: 'Cadence de tir',
    spread: 'Dispersion',
    speed: 'Vitesse',
    swing: 'Frappe',
    arc: 'Arc',
    reach: 'Portée',
    deflect: 'Déviation',
    yes: 'oui',
    no: 'non',
    vs: 'vs',
  },
  weapon: {
    blaster: { name: 'Blaster' },
    saber: { name: 'Sabre' },
    repeater: { name: 'Répéteur' },
    cannon: { name: 'Canon' },
    enemygun: { name: 'Fusil pillard' },
    enemyclaw: { name: 'Griffe pillarde' },
    enemymaul: { name: 'Maillet pillard' },
    enemynova: { name: 'Nova pillarde' },
    flamer: { name: 'Lance-flammes' },
    cryobolt: { name: 'Éclair de givre' },
    teslagun: { name: 'Fusil Tesla' },
    venomspit: { name: 'Cracheur de venin' },
    emberblade: { name: 'Lame de braise' },
    frostbrand: { name: 'Lame de givre' },
    stormglaive: { name: 'Glaive des tempêtes' },
    scattergun: { name: 'Fusil à dispersion' },
    seeker: { name: 'Chercheur' },
    mortar: { name: 'Mortier' },
    lasercutter: { name: 'Découpeur laser' },
    tomahawk: { name: 'Tomahawk' },
    hammer: { name: 'Marteau' },
    spear: { name: 'Lance' },
    novaburst: { name: 'Explosion Nova' },
    gyre: { name: 'Tourbillon' },
    carom: { name: 'Ricochet' },
    leech: { name: 'Sangsue' },
    cinderscatter: { name: 'Dispersion de braises' },
    frostseeker: { name: 'Chercheur de givre' },
  },
  skin: {
    vanguard: { name: 'Avant-garde' },
    skirmisher: { name: 'Tirailleur' },
    juggernaut: { name: 'Colosse' },
  },
  card: {
    potion_flow: { name: 'Flux vital', desc: 'Potions {factor}x plus fréquentes' },
    windfall: { name: 'Aubaine', desc: 'Les pièces valent {factor}x' },
    edge: { name: 'Tranchant', desc: '+{value}% de dégâts' },
    cadence: { name: 'Cadence', desc: '+{value}% de cadence de tir' },
    bulwark: { name: 'Rempart', desc: '+{value} PV max' },
    precision: { name: 'Précision', desc: '+{value}% de chance critique' },
    capacitor: { name: 'Condensateur', desc: '+{value} d’énergie max' },
    surge: { name: 'Surcharge', desc: 'L’énergie vaut {factor}x' },
    aegis: { name: 'Égide', desc: 'Les boucliers valent {factor}x' },
    stockpile: { name: 'Réserve', desc: 'Les matériaux valent {factor}x' },
    bounty: { name: 'Prime', desc: '+{count} arme par coffre' },
  },
  buff: {
    dmg_up: { name: 'Dégâts augmentés' },
    rof_up: { name: 'Cadence de tir augmentée' },
    vit_up: { name: 'Vitalité augmentée' },
    crit_up: { name: 'Chance de critique augmentée' },
    cell_up: { name: 'Énergie maximale augmentée' },
  },
  material: {
    physical: { name: 'Physique' },
    fire: { name: 'Feu' },
    ice: { name: 'Glace' },
    lightning: { name: 'Foudre' },
    poison: { name: 'Poison' },
  },
};
