 ...
 * module: one pure mapping from what the loop already holds to one object, no state of its own
 * and nothing to compose).
 *
 * It left `GameLoop` when that file hit exactly 500 lines and the chest caption needed one more
 * field (2026-09-15 — the field itself is gone again hours later, with the small chest's own
 * caption; see `ui/ChestPrompt.ts`). Moving the whole block rather than shaving a comment is
 * still the point: `HudContext` is a data shape that has grown a field roughly every time the
 * HUD grew a widget, and each field costs a doc comment saying WHY the loop answers that
 * question the way it does. Somewhere for those to live that is not the main loop is what the
 * next field needs too.
 */
...
export function buildHudContext(host: HudContextHost): HudContext {
  return {
