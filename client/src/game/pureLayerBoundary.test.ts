...
const PURE_FILES = [
  ...
  'match/roomCode.ts',
  // The per-mode party cap (2026-09-26, co-op room codes) — imported by the server through
  // the same `server/src/config.ts` seam as `roomCode.ts`, so the same argument applies.
  'match/partyShape.ts',
