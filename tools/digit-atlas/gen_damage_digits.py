...
Renders the ten digits of an OFL font into one white-fill, dark-outline PNG strip and writes the
...
def render(font_path: Path) -> tuple[Image.Image, dict]:
    ...
    boxes = [font.getbbox(d, stroke_width=STROKE_PX) for d in DIGITS]
    ...
    sheet = Image.new("RGBA", (pitch * len(DIGITS), cell_h + 2 * GUTTER_PX), (0, 0, 0, 0))
    draw = ImageDraw.Draw(sheet)
    frames = []
    for i, (d, b) in enumerate(zip(DIGITS, boxes)):
...
// {FONT_NAME} ({FONT_LICENSE}), white fill under a {STROKE_PX} px dark outline, one shared cell.
